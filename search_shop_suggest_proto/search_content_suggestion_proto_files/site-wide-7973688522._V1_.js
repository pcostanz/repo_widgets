 ...
 */
                ...
                if ((oldData = oldData[internalKey])) {
                    ...
                    if (events) {
                        ...
                        for (var type in events) {
                            for (var i = 0, l = events[type].length; 
                            i < l; 
                            i++) {
            ...
            jQuery.extend(jQuery.fx, {tick: function() {
                    for (var timers = jQuery.timers, i = 0; 
                    i < timers.length; 
                    ++i) {
                        if (!timers[i]()) {
                            timers.splice(i--, 1);
...
(function() {
    ...
    var doDeferred = function() {
        ...
        var callingCB;
        try {
            while (cbsWaiting.length && (new Date()).getTime() <= stopTime) {
    ...
    var initAmznJQ = function() {
        ...
        window.amznJQ = new function() {
            ...
            var enqueueEventCallback = function(eventName, cb) {
                if (!timesliceJS) {
                    $(document).one(eventName, cb);
                    return;
                }
                var queue = eventCBQueue[eventName] || [];
                queue.push(function() {
                    cb(jQuery.event.fix({type: eventName}));
                });
            ...
            (function() {
                ...
                me._getPLStat = function() {
                    return {H: hiPriUrls.length,L: lowPriUrls.length,P: plUrls.length,CL: currentLoaders,AL: allowedLoaders};
                };
...
(function() {
    ...
    function initAmazonPopover($) {
        ...
        var mouseTracker = function() {
            ...
            return {add: function(rectsArray, options) {
                    if (!listening) {
                        startListening();
                    }
        ...
        var iframePool = function() {
            ...
            return {checkout: function(jqObj) {
                    if (!pool.length) {
                        addToLib(1);
                    }
        ...
        var overlay = null;
        var popovers = [];
        var et = {MOUSE_ENTER: 1,MOUSE_LEAVE: 2,CLICK_TRIGGER: 4,CLICK_OUTSIDE: 8,fromStrings: function(s) {
                var flags = 0;
                var self = this;
                if (s) {
        ...
        var displayPopover = function(settings, triggers, destroyFunction) {
            ...
            if (settings.destination) {
                ...
                if (settings.cacheable !== false && ajaxCache[destinationUrl]) {
                    ...
                    $.ajax({url: destinationUrl,timeout: settings.ajaxTimeout,success: function(data) {
            ...
            if (closeEvent & et.MOUSE_LEAVE) {
                ...
                applyBacking(popover, {solidRectangle: settings.solidRectangle,useIFrame: settings.useIFrame,mouseEnter: function() {
                        if (timerID) {
                            clearTimeout(timerID);
                            timerID = null;
                        }
            ...
            if ($.browser.msie) {
                overlay.fitToScreen = function(e) {
                    ...
                    i++) {
                        ...
                        if (paneHeight > 0) {
                            if (i < children.size()) {
                                children.eq(i).css({width: windowWidth,height: paneHeight});
                            } else {
                                var slice = $("<div/>").css({opacity: 0.4,zIndex: z,width: windowWidth,height: paneHeight,top: (i * 2000)});
                ...
                overlay.snapToLeft = function() {
                    overlay.css("left", $(document).scrollLeft());
                };
        ...
        var locationFunction = {centered: function(b, popover, settings) {
...
(function(c) {
    ...
    function d(a) {
        var c = 50, b = function() {
            2E4 >= c && !a() && (setTimeout(b, c), c *= 2)
        };
        return b
    }
    function e() {
        return "object" === typeof c.P && "function" === typeof c.P.when && "function" === typeof c.P.register && "function" === typeof c.P.execute
    }
    function h(a) {
        var b = [], d = !1, e, f = !a ? i : function(a, b, d) {
            b.apply(c, d)
        }, a = function() {
            if (!d) {
                d = !0;
        ...
        a.watch = function(a, c) {
            c || (c = a, a = []);
            d ? f(a, c, e) : b.push({t: a,f: c})
        };
        a.isFired = function() {
            return d
        };
    ...
    function f(f) {
        var f = f || {bubble: !1}, g, k = {}, j = function(a) {
            return k[a] || (k[a] = h())
        };
        j.whenAvailable = function(a, b) {
            var d = h(), f = b.length;
            if (0 === f)
                return d(), d.watch;
            for (var e = 0, g = function() {
                e++;
                if (!(e < f)) {
                    for (var a = [], h = 0; h < f; h++)
                        a.push(j(b[h]).value()[0]);
                    d.apply(c, a)
                }
            }, k = 0; k < f; k++)
                j(b[k]).watch(a, g);
        ...
        g = j;
        var n = 0, v = {}, m = {declare: function(a, c) {
                v[a] || 
                (v[a] = {done: l()});
                g(a)(c)
            },build: function(a, c) {
                i([f.sourceName, a], function() {
                    m.declare(a, b(c)())
                })
            },getNow: function(a, c) {
                return g(a).value()[0] || c
            },publish: function(a, b) {
                m.declare(a, b);
                var d = f.parent;
                f.prefix && (a = f.prefix + "." + a);
                void 0 === d ? (c.amznJQ && c.amznJQ.declareAvailable(a), e() && c.P.register(a, function() {
                    return b
                })) : f.bubble && d.publish ? d.publish(a, b) : d.declare && d.declare(a, b)
            },importEvent: function(a, b) {
                var h = f.parent, b = b || {}, g = b.as || a, k = function(a) {
                    m.declare(g, void 0 === a || null === a ? b.otherwise : 
                    a)
                };
                if (h && h.when && h.declare && h.build)
                    h.when(a).run(k);
                else if (e() && c.P.when(a).execute(k), c.amznJQ)
                    c.amznJQ[f.useOnCompletion ? "onCompletion" : "available"](b.amznJQ || a, d(function() {
                        var a;
                        if (b.global) {
                            a = c;
                            for (var d = (b.global || "").split("."), f = 0, e = d.length; f < e; f++)
                                a && d[f] && (a = a[d[f]])
                        } else
                            a = b.otherwise;
                        if (b.retry && (void 0 === a || null === a))
                            return !1;
                        m.declare(g, a);
                        return !0
                    }))
            },stats: function(a) {
                var b = c.JSON.parse(c.JSON.stringify(v)), d;
                for (d in b)
                    if (b.hasOwnProperty(d)) {
                        for (var f = b[d], e = f.depend || [], h, k = 
                        0, l = [], j = 0; j < e.length; j++) {
                            var i = e[j];
                            g(i).isFired() ? v[i].done > k && (k = v[i].done, h = i) : l.push(i)
                        }
                        0 < l.length ? f.blocked = l : a ? delete b[d] : 0 < k && (f.longPole = h, f.afterLongPole = f.done - k)
                    }
                return b
            },when: function() {
                function c(b, e) {
                    var h = l(), k = [f.sourceName, b];
                    v[b] = {depend: d,registered: h};
                    g.whenAvailable(k, d)(k, function() {
                        v[b].done = l();
                        v[b].wait = v[b].done - h;
                        e.apply(this, a(arguments))
                    })
                }
                var d = a(arguments);
                return {run: function(a, b) {
                        b ? a = "run." + a : (b = a, a = "run.#" + n++);
                        c(a, b)
                        ...
                        c(d, function() {
                            var c = a(arguments), c = b(f).apply(this, c);
                            m.declare(d, c)
                        })
                    }}
            }};
    ...
    function g(a) {
        var a = a || "unknownSource", b = f({sourceName: a});
        b.declare("instance.sourceName", a);
        b.importEvent("jQuery", {as: "$",global: "jQuery"});
        b.importEvent("jQuery", {global: "jQuery"});
        b.importEvent("amznJQ.AboveTheFold", {as: "page.ATF",useOnCompletion: !0});
        b.importEvent("amznJQ.theFold", {as: "page.ATF",useOnCompletion: !0});
        b.importEvent("amznJQ.criticalFeature", 
        {as: "page.CF",useOnCompletion: !0});
        b.when("$").run("PageEventSetup", function(a) {
            var d = function() {
            ...
            a(c).load(d);
            "complete" === document.readyState ? d() : "interactive" === document.readyState && b.declare("page.domReady")
        });
        b.build("log", function() {
            return /[\?&]navDebug\b/.test(c.location.href) ? function(a) {
                c.console.log(a)
            } : function() {
            }
        });
    ...
    if (!c.$Nav || c.$Nav._replay) {
        ...
        Date.now || function() {
            ...
            try {
                for (; k[a] && 50 > a; )
                    a++, k[a - 1].f.apply(c, k[a - 1].a)
            } catch (b) {
                var d = b, f = "";
                k[a - 1].t && (f = "[" + k[a - 1].t.join(":") + "] ");
                d && d.message ? d.message = f + d.message : "object" === typeof b ? d.message = f : d = f + d;
                if (c.ueLogError && c.console && c.console.error)
                    c.ueLogError(d), c.console.error(d);
                else
                    throw d;
            }finally {
                k = k.slice(a), 0 < k.length ? setTimeout(j, 1) : n = !1
            }
        }, k = [], n;
        i = function(b, c, d) {
            k.push({t: b,f: c,a: a(d || [])});
            n || (setTimeout(j, 1), n = !0)
        };
        c.$Nav || (c.$Nav = g("rcx-nav"));
        if (c.$Nav.make) {
            if (c.$Nav.make._shims) {
                for (var m = 0, s = c.$Nav.make._shims.length; m < s; m++) {
                    var u = c.$Nav.make._shims[m], q = g(u._sourceName);
                    if (u._replay) {
                        for (var p = 0, A = u._replay.length; p < A; p++)
                            for (var y = q, B = u._replay[p], r = 0, E = B.length; r < E; r++) {
                                var v = B[r];
                                if (!y[v.m])
                                    break;
                                y = y[v.m].apply(y, v.a)
            ...
            c.$Nav.make = g;
        c.$Nav.declare("depend", f);
        c.$Nav.declare("promise", h);
        c.$Nav.declare("argArray", a);
        c.$Nav.declare("schedule", 
        i);
        c.$Nav.declare("now", l)
    }
})(window);
window.$Nav.when("argArray").build("$F", function(c) {
    function a(a, b) {
        this.up = a;
        this.action = b
    }
    function b(b) {
        return function() {
            ...
            return new a(this, function(a) {
                return b.apply(this, [a].concat(e))
            })
        }
    }
    ...
    a.prototype.partial = b(function(a) {
        ...
        return function() {
            return a.apply(this, b.concat(c(arguments)))
        }
    });
    a.prototype.rpartial = b(function(a) {
        var b = Array.prototype.slice.call(arguments, 1);
        return function() {
            return a.apply(this, c(arguments).concat(b))
        }
    });
    a.prototype.throttle = b(function(a, 
    b) {
        function c() {
            g ? (g = !1, setTimeout(c, b), a()) : f = !1
        }
        var f = !1, g = !1;
        return function() {
            f ? g = !0 : (f = !0, setTimeout(c, b), a())
        }
    });
    ...
    a.prototype.tap = b(function(a, b) {
        ...
        return function() {
            b.apply(this, h.concat(c(arguments)));
            return a.apply(this, arguments)
        }
    });
    return new a
...
(function(c, a) {
    a.when("$").run("ImportPopovers", function(b) {
        a.importEvent("legacy-popover", {as: "$popover",amznJQ: "popover",otherwise: b})
    });
    a.importEvent("iss", {amznJQ: "search-js-autocomplete",global: "iss",retry: !0});
    a.when("nav.inline").declare("config.newFlyouts", !1);
    if (c.amznJQ) {
        var b = c.amznJQ;
        b.available("navbarInline", function() {
            a.declare("logEvent", c._navbar.logEv);
            a.declare("config.readyOnATF", c._navbar.readyOnATF);
            a.declare("config.browsePromos", c._navbar.browsepromos);
            a.declare("config.yourAccountPrimeURL", 
            c._navbar.yourAccountPrimer);
            a.declare("config.sbd", c._navbar._sbd_config);
            a.declare("config.responsiveGW", !!c._navbar.responsivegw);
            a.declare("config.swmStyleData", c._navbar._swmStyleData || {});
            a.declare("config.dismissNotificationUrl", c._navbar.dismissNotificationUrl);
            a.declare("config.signOutText", c._navbar.signOutText);
            a.declare("config.lightningDeals", c._navbar._lightningDealsData || {});
            a.declare("config.enableDynamicMenus", c._navbar.dynamicMenus);
            a.declare("config.dynamicMenuArgs", c._navbar.dynamicMenuArgs || 
            {});
            a.declare("config.dynamicMenuUrl", c._navbar.dynamicMenuUrl);
            a.declare("config.ajaxProximity", c._navbar._ajaxProximity);
            a.declare("config.recordEvUrl", c._navbar.recordEvUrl);
            a.declare("config.recordEvInterval", c._navbar.recordEvInterval || 6E4);
            a.declare("config.sessionId", c._navbar.sid);
            a.declare("config.requestId", c._navbar.rid);
            a.declare("config.autoFocus", c._navbar.enableAutoFocus)
        });
        b.available("navbarBTF", function() {
            a.declare("config.flyoutURL", c._navbar.flyoutURL);
            a.declare("config.prefetch", 
            c._navbar.prefetch)
        });
        a.importEvent("navbarBTF", {as: "btf.full"});
        a.importEvent("navbarBTFLite", {as: "btf.lite"});
        a.importEvent("navbarInline", {as: "nav.inline"});
        a.when("nav.inline", "api.unHideSWM", "api.exposeSBD", "api.navDimensions", "api.onShowProceedToCheckout", "api.update", "api.setCartCount", "api.getLightningDealsData", "api.overrideCartButtonClick", "ajax.flyoutDispatcher", "flyout.shopall", "flyout.wishlist", "flyout.cart", "flyout.youraccount").publish("navbarJSInteraction");
        a.when("navbarJSInteraction").publish("navbarJS-beacon");
        a.when("navbarJSInteraction").publish("navbarJS-jQuery")
    }
})(window, window.$Nav);
window.$Nav.when("$", "now").build("MouseTracker", function(c) {
    function a(a) {
        e.push({x: a.pageX,y: a.pageY,when: a.timeStamp});
        100 < e.length && (e = e.slice(-d))
    }
    function b() {
        this.active = !0;
        0 === h && c(document).mousemove(a);
        h++
    }
    var d = 10, e = [], h = 0;
    b.prototype.stop = function() {
        this.active && (h--, 0 === h && c(document).unbind("mousemove", a))
    };
    b.prototype.position = function() {
        var a = e.length;
        return !a ? null : c.extend(!0, {}, e[a - 1])
    };
    b.prototype.history = function(a) {
        var b = e.length;
        if (0 === b)
            return [];
        var h = Math.min(b, d);
        0 < arguments.length && 
        (h = Math.min(h, a));
        for (var i = [], j = b - 1, b = b - h; j >= b; j--)
            i.push(c.extend(!0, {}, e[j]));
        return i
    };
...
window.$Nav.when("$", "MouseTracker").build("Proximity", function(c, a) {
    return {onEnter: function(b, d, e) {
            function h() {
                n && (window.clearInterval(n), n = null);
                f.stop();
                g = f = null
            }
            var f = a.start(), g, l = [], i = [], j = [], k = [];
            b.each(function(a, b) {
                var b = c(b), d = b.offset();
                l.push(d.left);
                j.push(d.top);
                i.push(d.left + b.width());
                k.push(d.top + b.height())
            });
            g = {left: Math.min.apply(Math, l) - d[3],top: Math.min.apply(Math, j) - d[0],right: Math.max.apply(Math, i) + d[1],bottom: Math.max.apply(Math, k) + d[2]};
            var n = window.setInterval(function() {
                var a = 
                f.position();
                a && (a.x >= g.left && a.x <= g.right && a.y >= g.top && a.y <= g.bottom) && (h(), e())
            }, 100);
...
window.$Nav.when("$", "$F", "promise", "MouseTracker").build("flyout.MouseIntent", function(c, a, b, d) {
    function e(a, b, c, d) {
        var e = (c.x - b.x) * (d.y - b.y) - (d.x - b.x) * (c.y - b.y), h = ((d.x - a.x) * (b.y - a.y) - (b.x - a.x) * (d.y - a.y)) / e, b = ((b.x - a.x) * (c.y - a.y) - (c.x - a.x) * (b.y - a.y)) / e;
        return 0 < ((c.x - a.x) * (d.y - a.y) - (d.x - a.x) * (c.y - a.y)) / e && 0 < h && 0 < b
    }
    function h(f, e) {
        var h, i, f = c(f), e = e || {};
        this._onStrayEvent = b();
        this._onArriveEvent = b();
        this._tracker = d.start();
        this.onArrive(a.bind(this._tracker).on(this._tracker.stop));
        this.onStray(a.bind(this._tracker).on(this._tracker.stop));
        this._minorDelay = e.minorDelay || 200;
        this._majorDelay = e.majorDelay || 100;
        i = f;
        var j = e.slop || 25, k = i.offset();
        h = {x: k.left,y: k.top - j};
        i = {x: k.left + i.outerWidth(),y: k.top + i.outerWidth() + j};
        this._upperLeft = h;
        this._lowerRight = i;
        this._minor = 0;
        this._tick()
    }
    h.prototype.onArrive = function(a) {
        this._onArriveEvent.watch(a);
        return this
    };
    h.prototype.onStray = function(a) {
        this._onStrayEvent.watch(a);
        return this
    };
    h.prototype._scheduleTick = function(b) {
        this._timer = a.delay(b).bind(this).run(this._tick)
    };
    ...
    h.prototype._tick = function() {
        if (!this._onStrayEvent.isFired() && !this._onArriveEvent.isFired()) {
            var a = this._tracker.history(2);
            if (2 > a.length)
                this._scheduleTick(this._minorDelay);
            else {
                var b = a[0], a = a[1];
                b.x >= this._upperLeft.x && b.x <= this._lowerRight.x && b.y >= this._upperLeft.y && b.y <= this._lowerRight.y ? this._onArriveEvent() : !e(b, a, this._upperLeft, this._lowerRight) && !e(b, a, {x: this._lowerRight.x,y: this._upperLeft.y}, {x: this._upperLeft.x,y: this._lowerRight.y}) ? this._onStrayEvent() : 
                2 > Math.abs(b.x - a.x) && 2 > Math.abs(b.y - a.y) ? 2 <= this._minor ? this._onStrayEvent() : (this._minor++, this._scheduleTick(this._minorDelay)) : (this._minor = 0, this._scheduleTick(this._majorDelay))
            }
        }
    };
    return h
});
window.$Nav.when("$F", "flyout.EventStream", "flyout.MouseIntent").build("flyout.SloppyTrigger", function(c, a, b) {
    function d() {
    }
    d.prototype.register = function(a, b, d) {
        a.mouseover(c.bind(this).partial(d, b).on(this._show))
    };
    d.prototype._show = function(a, d) {
        var f = {flyout: a,content: d};
        this._shown ? this._shown.flyout !== a && (this._toShow = f, this._intent || (this._intent = (new b(this._shown.content)).onArrive(c.bind(this).on(function() {
            this._toShow = this._intent = null
        })).onStray(c.bind(this).on(function() {
            this._intent = 
            null;
            this._shown.flyout.hide();
            this._toShow.flyout.show();
            this._shown = this._toShow;
            this._toShow = null
        })))) : (this._shown = f, a.show())
    };
    d.prototype.reset = function() {
        this._intent && (this._intent.cancel(), this._intent = null);
        this._shown && (this._shown.flyout.hide(), this._shown = null)
    };
    return d
});
window.$Nav.when("$F", "argArray").build("flyout.EventStream", function(c, a) {
    function b() {
        this._observers = []
    }
    b.prototype.trigger = function() {
        for (var b = a(arguments), c = 0, h = this._observers.length; c < h; c++)
            this._observers[c].apply(window, b)
    };
    b.prototype.observe = function(a) {
        if ("function" !== typeof a)
            throw "Can't register a non-function observer";
        this._observers.push(a)
    };
    b.prototype.boundObserve = function() {
        return c.bind(this).on(this.observe)
    };
    return b
});
window.$Nav.when("$", "$byID", "agent").build("flyout.fullWidthPosition", function(c, a, b) {
    return function(c) {
        return {align: function(e) {
                c.align(e);
                e.css({left: "0",right: "0","border-radius": "0px","border-right": "0px","border-left": "0px","padding-left": "0px","padding-right": "0px","min-width": "1024px"});
                b.quirks && e.css({width: a("navbar").width() + "px"})
            }}
    }
});
window.$Nav.when("$", "$F", "agent").build("flyout.position", function(c) {
    function a(a, b) {
        return parseInt(a.css(b) || "0", 10) || 0
    }
    function b(a, b) {
        a = a.split(" ");
        if (!d[a[0]])
            throw "Invalid vertical descriptor: " + a[0];
        e[a[1]] && (this.horiz = e[a[1]]);
        this.vert = d[a[0]];
        this.nudge = {x: b && b.x ? b.x : 0,y: b && b.y ? b.y : 0}
    }
    var d = {top: {offset: function(a) {
                return a.offset().top
            },align: function(b, c) {
                return {top: c - b.offset().top + b.position().top + a(b, "margin-top")}
            }},middle: {offset: function(a) {
                return a.offset().top + a.outerHeight() / 
                2
            },align: function(a, b) {
                return {top: b - a.offset().top + a.position().top - a.outerHeight() / 2}
            }},bottom: {offset: function(a) {
                return a.offset().top + a.outerHeight()
            },align: function(b, c) {
                return {top: c - b.offset().top + b.position().top - b.outerHeight() + a(b, "margin-bottom")}
            }}}, e = {left: {offset: function(a) {
                return a.offset().left
            },align: function(b, c) {
                return {left: c - b.offset().left + b.position().left + a(b, "margin-left")}
            }},right: {offset: function(a) {
                return a.offset().left + a.outerWidth()
            },align: function(b, d) {
                var e;
                e = document.documentElement.clientWidth ? 
                document.documentElement.clientWidth : c(window).width();
                return {right: e - d + a(b, "margin-right")}
            }}};
    b.prototype.on = function(a) {
        a = c(a);
        return {x: this.horiz ? this.horiz.offset(a) + this.nudge.x : this.nudge.x,y: this.vert.offset(a) + this.nudge.y}
    };
    b.prototype.alignment = function(a, b) {
        return c.extend({}, this.horiz ? this.horiz.align(a, b.x + this.nudge.x) : {left: this.nudge.x}, this.vert.align(a, b.y + this.nudge.y))
    };
    b.prototype.to = function(a, d) {
        var e = new b(a, d), l = this;
        return {usingBase: function(a) {
                return {align: function(b) {
                        var b = 
                        c(b), d = l.on(a);
                        b.css("position", "absolute");
                        b.css(e.alignment(b, d))
                    }}
            }}
    };
    return function(a, c) {
...
window.$Nav.when("$", "$F", "argArray", "promise", "flyout.EventStream").build("flyout.Engine", function(c, a, b, d, e) {
    function h(a) {
        var b;
        b = "object" === typeof window.Node ? a instanceof window.Node : a && "object" === typeof a && "number" === typeof a.nodeType && "string" === typeof a.nodeName;
        if (b || a instanceof c)
            return a;
        if ("function" === typeof a)
            return h(a());
        throw "Unsupported flyout content type";
    }
    function f(b) {
        this._renderEvents = d(!0);
        this.onRender = this._renderEvents.watch;
        this._showEvents = new e;
        this.onShow = this._showEvents.boundObserve();
        this._hideEvents = new e;
        this.onHide = this._hideEvents.boundObserve();
        this.aligner = b.align || function() {
        };
        this.elem = a.memoize().on(function() {
            var a = c(h(b.content));
            a.css(b.css || {});
            this._renderEvents(a);
            return a
        });
        c(window).bind("resize", a.bind(this).on(this.align))
    }
    f.prototype.align = function() {
        this._renderEvents.isFired() && this.isVisible() && (this.aligner.align ? this.aligner.align(this.elem()) : "function" === typeof this.align && this.aligner(this.elem()))
    };
    f.prototype.isVisible = function() {
        return this._renderEvents.isFired() && 
        this.elem().is(":visible")
    };
    f.prototype.show = function() {
        this.isVisible() || (this.elem().show(), this.align(), this._showEvents.trigger(this))
    };
    f.prototype.hide = function() {
        this.isVisible() && (this.elem().hide(), this._hideEvents.trigger(this))
    };
    f.prototype.onInteract = function(b) {
        function c() {
...
window.$Nav.when("$", "$F").build("flyout.DelayTrigger", function(c, a) {
    function b(a, b) {
        this.timeout = a;
        this.inner = b
    }
    b.prototype.show = function() {
        this.hideTimer && (window.clearTimeout(this.hideTimer), this.hideTimer = null);
        this.showTimer || (this.showTimer = a.delay(this.timeout).bind(this).run(function() {
            this.showTimer = null;
            this.inner.show()
        }))
    };
    b.prototype.hide = function() {
        this.showTimer && (window.clearTimeout(this.showTimer), this.showTimer = null);
        this.hideTimer || (this.hideTimer = a.delay(this.timeout).bind(this).run(function() {
            this.hideTimer = 
            null;
            this.inner.hide()
        }))
    };
    b.prototype.onHoverOf = function(b) {
        c(b).hover(a.bind(this).on(this.show), a.bind(this).on(this.hide));
        return this
    };
    b.hover = function(c, e, h) {
        e = new b(e, c);
        e.onHoverOf(h);
        c.onRender(a.bind(e).on(e.onHoverOf));
        return e
    };
    return b
});
window.$Nav.when("$", "$F", "agent").build("flyout.ClickTrigger", function(c, a) {
    function b(a, b, d) {
        var g = b.length;
        0 === c(a.target).parents().add(a.target).filter(function() {
            for (var a = 0; a < g; a++)
                if (this === b[a])
                    return !0;
            return !1
    ...
    function d(d) {
        this.inner = d;
        this.elems = [];
        this.bodyHandler = a.rpartial(this.elems, d).on(b);
        d.onHide(function() {
            c(document.body).unbind("click", this.bodyHandler)
        })
    }
    d.prototype.ignoreClicks = function(a) {
        this.elems.push(c(a).get(0));
        return this
    };
    d.prototype.showOnClick = 
    function(b) {
        this.ignoreClicks(b);
        c(b).click(a.bind(this).on(function(a) {
            if (0 === a.button || void 0 === a.button)
                this.inner.isVisible() ? this.inner.hide() : this.inner.show(), this.inner.isVisible() && c(document.body).click(this.bodyHandler)
        }));
        return this
    };
...
window.$Nav.when("$", "$F", "now").build("flyout.FastGuardTrigger", function(c, a, b) {
    function d(b) {
        this.inner = b;
        this.lastInteract = 0;
        this.isVisible = a.bind(b).on(b.isVisible);
        this.onRender = a.bind(b).on(b.onRender);
        this.onHide = a.bind(b).on(b.onHide);
        this.onShow = a.bind(b).on(b.onShow)
    }
    d.prototype.interacted = function() {
        this.lastInteract = b()
    };
    ...
    d.prototype.hide = function() {
        400 < b() - this.lastInteract && this.inner.hide()
    };
    return d
});
window.$Nav.when("$F", "flyout.EventStream").build("flyout.LockTrigger", function(c, a) {
    function b(b) {
        this.locked = !1;
        this.inner = b;
        this.isVisible = c.bind(b).on(b.isVisible);
        this.onHide = c.bind(b).on(b.onHide);
        this.onShow = c.bind(b).on(b.onShow);
        this.onRender = c.bind(b).on(b.onRender);
        this._onHideRequestEvents = new a;
        this.onHideRequest = this._onHideRequestEvents.boundObserve()
    }
    b.prototype.show = function() {
        this.locked ? this.visible = !0 : this.inner.show();
        return this
    };
    b.prototype.hide = function() {
        this._onHideRequestEvents.trigger(this);
        this.locked ? this.visible = !1 : this.inner.hide();
        return this
    };
    b.prototype.lock = function() {
        this.locked = !0;
        this.visible = this.inner.isVisible()
    };
    b.prototype.unlock = function() {
        this.locked = !1;
        this.visible ? this.inner.show() : this.inner.hide()
    };
    return b
});
window.$Nav.when("$", "agent", "flyout.onClickDebug", "flyout.LockTrigger", "flyout.FastGuardTrigger").build("flyout.setupTriggers", function(c, a, b, d, e) {
    return function(h, f, g) {
        ...
        if (a.touch) {
            ...
            h.onRender(function(a) {
                c(a).bind("mousedown mouseup", function() {
                    i.interacted()
                })
            })
        } else
...
window.$Nav.when("$F", "argArray", "flyout.EventStream").build("flyout.Group", function(c, a, b) {
    function d() {
        this._grouped = [];
        this._showEvents = new b;
        this.onShow = this._showEvents.boundObserve()
    }
    d.prototype.register = function(b) {
        this._grouped.push(b);
        var d = a(arguments);
        b.onShow(c.bind(this).on(function() {
            for (var a = 0, c = this._grouped.length; a < c; a++)
                this._grouped[a] !== b && this._grouped[a].hide();
            this._showEvents.trigger.apply(this._showEvents, d)
        }));
        return b
    };
    d.prototype.hide = function() {
        for (var a = 0, b = this._grouped.length; a < 
        b; a++)
            this._grouped[a].hide()
    };
    return d
});
window.$Nav.when("$", "byID", "agent").build("flyout.iframeShim", function(c, a, b) {
    function d() {
        if (b.ie6) {
            if (window.AmazonPopoverImages.pixel)
                return window.AmazonPopoverImages.pixel;
            var c = a("nav_trans_pixel");
            if (c)
                return c.src
        }
        return "javascript:void(false)"
    }
    function e() {
        ...
        a.tabIndex = -1;
        a.src = d();
        f ? f.appendChild(a) : (f = document.createElement("div"), 
        f.appendChild(a), document.body.appendChild(f));
        return c(a)
    }
    function h(a, b) {
        b.css({display: "block",top: a.offset().top,left: a.offset().left,width: a.outerWidth(),height: a.outerHeight(),zIndex: parseInt(a.css("z-index"), 10) - 1})
    }
    if (!b.ie6)
        return function() {
            return {checkout: function() {
                    return {realign: function() {
                        },release: function() {
                        }}
                }}
        };
    var f, g = [];
    window.$Nav.when("page.CF").run("iframePool", function() {
        g.push(e())
    });
    return function() {
        var a;
        return {checkout: function(b) {
                a && a.release();
                var d = 0 < g.length ? g.pop() : 
                e(), f = b, f = c(f);
                h(f, d);
                var n = !1;
                return a = {realign: function() {
                        h(f, d)
                    },release: function() {
                        n || (n = !0, d.css("display", "none"), g.push(d))
                    }}
            }}
    }
});
window.$Nav.when("api.publish").build("flyout.registry", function(c) {
    var a = {};
    c("flyout", function(b) {
        return a[b]
    });
    c("lockFlyouts", function(b) {
        for (var c in a)
            a.hasOwnProperty(c) && (b && a[c].hide(), a[c].lock())
    });
    c("unlockFlyouts", function() {
        for (var b in a)
            a.hasOwnProperty(b) && a[b].unlock()
    });
    return function(b, c) {
        return a[b] = c
    }
});
window.$Nav.when("$F", "promise", "api.publish").build("flyout.onClickDebug", function(c, a, b) {
    a = a(!0);
    b("debugFlyoutClick", c.on(a));
    /[\?&]navFlyoutClick\b/.test(window.location.href) ? a() : /\bnavFlyoutClick=/.test(document.cookie) && a();
    return a.watch
});
window.$Nav.when("$", "$F", "areaMapper", "agent", "logEvent", "recordEv", "isAUI", "dismissTooltip", "flyout.Engine", "flyout.position", "flyout.Group", "flyout.onClickDebug", "flyout.DelayTrigger", "flyout.ClickTrigger", "flyout.LockTrigger", "flyout.setupTriggers", "flyout.registry", "flyout.iframeShim", "flyout.fullWidthPosition", "nav.inline").build("flyout.create", function(c, a, b, d, e, h, f, g, l, i, j, k, n, m, s, u, q, p, A) {
    function y(a, b, e) {
        a = a.get(0);
        ...
        if (e)
            ...
            for (e.className = "nav-flyout-borders"; a.firstChild; ) {
                var k = a.firstChild;
                a.removeChild(k);
                e.appendChild(k)
            }
        ...
        a.style.display = "none";
        f() || (a.className += " nav-beacon-flyout nav-css3");
        e = f() ? "nav-aui" : "nav-css3";
        a.className += " " + e + (d.ie ? "-ie" : "-not-ie");
        d.ie6 && (a.className += " " + e + "-ie6");
        b && (a.className += " " + b.join(" "));
    ...
    new j, E = p();
    return function(f, j) {
        var p = c(j.button), z = c(j.content);
        if (p.length && z.length) {
            p.addClass("nav-menu-active").removeClass("nav-menu-inactive");
            var t = a.partial(z, j.classes, j.skin).on(y), D = c.extend({zIndex: 1E3}, j.css || {}), x = j.align, F = j.fullWidth;
            if (!x || "string" === typeof x)
                F && (x = null), x = i("bottom " + x).to("top " + x);
            x = x.usingBase(p);
            F && (x = A(x));
            var w = new l({content: t,css: D,align: x});
            !1 !== j.markOpen && (w.onShow(function() {
                p.addClass("nav-button-outer-open")
            }), w.onHide(function() {
                p.removeClass("nav-button-outer-open")
            }));
            w.onShow(function() {
                b.disable(z)
            });
            ...
            w.onShow(function() {
                if (!j.blur || j.blur())
                    window.$Nav.getNow("iss", {blur: function() {
                        }}).blur(), g()
            });
            w.onInteract(function() {
                j.event && e("string" === typeof j.event ? {t: j.event} : j.event);
                w.elem().find("li[data-nav-wt]").andSelf().each(function() {
                    var a = c(this), b = a.attr("data-nav-wt");
                    b && (h(b), a.removeAttr("data-nav-wt"))
                })
            });
            var G;
            w.onShow(function() {
            ...
            r.register(t);
            D = 
            a.memoize().on(function() {
                p.click(function(a) {
                    (0 === a.button || void 0 === a.button) && a.preventDefault()
                })
            });
            j.noClick || d.touch ? D() : k(D);
            u(t, function(a) {
                n.hover(a, 250, p)
            }, function(a) {
                m.build(a, p)
            });
            if (j.ajaxFlyoutDispatcher)
                w.onShow(j.ajaxFlyoutDispatcher.loadDynamicMenusConditionally);
            w.onRender(function(a) {
                a.append('<div class="nav-arrow">').append('<div class="nav-arrow-inner">')
            });
            return q(f, {elem: a.bind(w).on(w.elem),onShow: a.bind(w).on(w.onShow),onHide: a.bind(w).on(w.onHide),onHideRequest: a.bind(t).on(t.onHideRequest),
                onRender: a.bind(w).on(w.onRender),onInteract: a.bind(w).on(w.onInteract),show: a.bind(t).on(t.show),hide: a.bind(t).on(t.hide),lock: a.bind(t).on(t.lock),unlock: a.bind(t).on(t.unlock),resized: function() {
                    G && G.realign()
                }})
        }
    }
...
(function(c) {
    c.when("$").build("isAUI", function(a) {
        return function() {
            return 0 < a("#navbar.nav-aui").length
        }
    });
    c.when("$").build("first", function(a) {
        return function(b, c, e) {
            return a(b).filter("[" + c + "='" + e + "']").filter(":first")
        }
    });
    c.when("$").build("agent", function(a) {
        return new function() {
            function b() {
                var a = Array.prototype.slice.call(arguments, 0);
                return RegExp("(" + a.join("|") + ")", "i").test(navigator.userAgent)
            }
            this.iPhone = b("iPhone");
            this.iPad = b("iPad");
            this.kindleFire = b("Kindle Fire", "Silk/");
            this.android = 
            b("Android");
            this.webkit = b("WebKit");
            this.ie10 = b("MSIE 10");
            this.ie6 = a.browser.msie && 6 >= parseInt(a.browser.version, 10);
            this.ie = a.browser.msie;
            this.opera = b("Opera");
            this.touch = this.iPhone || this.iPad || this.android || this.kindleFire || !!("ontouchstart" in window) || 0 < window.navigator.msMaxTouchPoints || b("\\bTouch\\b");
            this.ie10touch = this.ie10 && this.touch;
            this.mac = b("Macintosh");
            this.iOS = this.iPhone || this.iPad;
            this.quirks = this.ie && document && "CSS1Compat" !== document.compatMode
        }
    });
    c.declare("byID", function(a) {
        return document.getElementById(a)
    });
    c.when("log").build("dismissTooltip", function(a) {
        var b = !1;
        return function() {
            b || (a("Tooltip dismissed"), c.publish("navDismissTooltip"), b = !0)
        }
    });
    c.when("log").build("recordEv", function(a) {
        var b = [], d = {};
        c.when("$", "config.recordEvUrl", "config.recordEvInterval", "config.sessionId", "config.requestId").run("recordEvLoop", function(a, c, d, g, l) {
            function i(a) {
                j++;
                if (0 !== b.length) {
                    var a = a || j, d = b.join(":"), d = window.encodeURIComponent(d), e = window.ue && window.ue.sid, e = e || g, d = "trigger=" + d + "&when=" + a + "&sid=" + (e ? e : "") + 
                    "&rid=", a = (a = window.ue && window.ue.rid) || l, d = d + (a ? a : ""), a = 0 < c.indexOf("?") ? "&" : "?";
                    (new Image).src = c + a + d;
                    b = []
                }
            }
        ...
        return function(c) {
            c in d || (b.push(c), a("Recording Event: " + c), d[c] = !0)
        }
    });
    c.build("eachDescendant", function() {
        function a(b, c) {
            c(b);
            for (b = b.firstChild; b; )
                a(b, c), b = b.nextSibling
        }
        return a
    });
    c.when("log", "promise", "$F").run("pageReady", function(a, b, d) {
        function e() {
            return "complete" != document.readyState
        }
        function h() {
            return !!c.getNow("config.readyOnATF")
        }
        b = b();
        b.watch([c.getNow("instance.sourceName"), "pageReadyCallback"], function(b) {
            a("page.ready triggered by: " + b);
            c.declare("page.ready")
        });
        "complete" === document.readyState ? b("immediate") : (c.when("page.ATF").run("page.TriggerATF", d.partial("Event: page.ATF").tap(a).iff(e).iff(h).on(b)), c.when("page.CF").run("page.TriggerCF", d.partial("Event: page.CF").tap(a).iff(e).on(b)), c.when("page.domReady").run("page.TriggerDom+", d.delay(1E4).partial("Event: page.domReady+").tap(a).iff(e).on(b)), 
        c.when("page.loaded").run("page.TriggerLoaded", d.delay(100).partial("Event: page.loaded+").tap(a).on(b)))
    });
    c.when("$", "agent").build("onOptionClick", function(a, b) {
        return function(c, e) {
            var h = a(c);
            if (b.mac && b.webkit || b.touch && !b.ie10)
                ...
                var f = {click: 0,change: 0}, g = function(a, b) {
                    return function() {
                        f[a] = (new Date).getTime();
                        100 >= f[a] - f[b] && e.apply(h)
                    }
                };
                h.click(g("click", "change")).change(g("change", "click"))
            }
        }
    });
    c.when("$", "promise", "api.publish").build("triggerProceedToCheckout", 
    function(a, b, d) {
        var e = b();
        d("onShowProceedToCheckout", function(b) {
            e.watch([c.getNow("instance.sourceName"), "onShowProceedToCheckoutCallback"], function() {
                var c = a("#nav_cart_flyout a[href~=proceedToCheckout]");
                0 < c.length && b(c)
            })
        });
        return e
    });
    c.when("promise", "api.publish").run("triggerNearFlyout", function(a, b) {
        var d = a();
        b("onNearFlyout", function(a) {
            d.watch([c.getNow("instance.sourceName"), "onNearFlyoutCallback"], a)
        });
        c.when("config.prefetchUrls", "event.prefetch").run("prefetch", function(a) {
            window.amznJQ && 
            window.amznJQ.addPL(a);
            d()
        })
    });
    c.when("$", "byID").build("$byID", function(a, b) {
        return function(c) {
            return a(b(c) || [])
        }
    });
    c.when("$byID", "config.dynamicMenuArgs", "btf.full").build("flyout.primeAjax", function(a, b) {
        return b.hasOwnProperty("isPrime") && a("nav-prime-menu").hasClass("nav-ajax-prime-menu")
    });
    c.when("$", "agent").build("areaMapper", function(a, b) {
        var c = function() {
        };
        if (!b.kindleFire)
            return {disable: c,enable: c};
        var e = a([]);
        return {disable: function(b) {
                var c = a("img[usemap]").filter(function() {
                    return 0 === 
                    a(this).parents(b).length
                });
                e = e.add(c);
                c.each(function() {
    ...
    c.when("$").build("template", function(a) {
        var b = {};
        return function(c, e) {
            ...
            "',$1,'").split("<#").join("');").split("#>").join("p.push('") + "');}return p.join('');"));
            try {
                return e.jQuery = a, b[c](e)
            } catch (h) {
                return ""
            }
        }
    });
    c.when("$", "config.yourAccountPrimeURL", "nav.inline").run("yourAccountPrimer", function(a, b) {
        b && a("#nav_prefetch_yourorders").mousedown(function() {
            var a = !1;
            return function() {
                a || (a = !0, (new Image).src = b)
            }
        }())
    });
    c.when("$", "byID", "agent", "dismissTooltip", "recordEv").build("flyout.NavButton", function(a, b, c, e, h) {
        return function(f) {
            this._jqId = "#" + f;
            var g = this;
            b(f).className = 
            b(f).className.replace("nav-menu-inactive", "nav-menu-active");
            var l = 0, i;
            this.onShow = function() {
                l = +new Date;
                e();
                g._toggleClass(!0, "nav-button-outer-open");
                i && h(i)
            };
            this.onHide = function() {
                g._toggleClass(!1, "nav-button-outer-open")
            };
            this.registerTrigger = function(b) {
                var e = this._defaultTriggerParams(g);
                a().extend(e, b || {});
                var f = a(g._jqId).amazonPopoverTrigger(e);
                e.localContent && (i = a(e.localContent).attr("data-nav-wt"));
                c.touch && a.AmazonPopover.support.controlCallbacks && a(g._jqId).click(function() {
                    f.amznPopoverVisible() && 
                    400 < +new Date - l && f.amznPopoverHide()
                })
            };
            this.removeTrigger = function() {
                a(g._jqId).removeAmazonPopoverTrigger()
            };
            this._toggleClass = function(b, c) {
                b ? a(g._jqId).addClass(c) : a(g._jqId).removeClass(c)
            };
            a(g._jqId).keypress(function(b) {
                13 == b.which && a(g._jqId).attr("href") && (window.location = a(g._jqId).attr("href"))
            });
            this._defaultTriggerParams = function(a) {
                a = {width: null,location: "bottom",locationAlign: "left",locationMargin: 0,hoverShowDelay: c.touch ? 0 : 250,hoverHideDelay: c.touch ? 0 : 250,showOnHover: !0,forceAlignment: !0,
                    focusOnShow: !1,skin: null,onShow: a.onShow,onHide: a.onHide,showCloseButton: !1,group: "navbar"};
                c.ie10touch && (a.hoverShowDelay = 250);
                return a
            }
        }
    });
    c.when("$byID", "agent", "$popover").build("flyout.SKIN", function(a, b, c) {
        return function(e, h, f) {
            var g = function() {
                var c = a("nav-bar-outer").width(), c = Math.min(30, Math.max(0, c - e.offset().left - e.outerWidth() - h)), d = 30 > c ? ' style="width: ' + (c + 15) + 'px;"' : "", g = ["nav_flyout_table", "nav-beacon-flyout"];
                b.ie6 && g.push("nav_ie6");
                f && g.push(f);
                return '<table cellspacing="0" cellpadding="0" surround="0,' + 
                c + ',30,30" class="' + g.join(" ") + '"><tr><td class="nav_pop_tl nav_pop_h"><div class="nav_pop_lr_min"></div></td><td class="nav_pop_tc nav_pop_h"></td><td class="nav_pop_tr nav_pop_h"' + d + '><div class="nav_pop_lr_min"' + d + '></div></td></tr><tr><td class="nav_pop_cl nav_pop_v"></td><td class="nav_pop_cc ap_content"></td><td class="nav_pop_cr nav_pop_v"' + d + '></td></tr><tr><td class="nav_pop_bl nav_pop_v"></td><td class="nav_pop_bc nav_pop_h"></td><td class="nav_pop_br nav_pop_v"' + d + "></td></tr></table>"
            };
            return c.AmazonPopover.support && c.AmazonPopover.support.skinCallback ? g : g()
        }
    });
    c.when("$", "byID", "eachDescendant").build("flyout.computeFlyoutHeight", function(a, b, c) {
        var e = {};
        return function(h) {
            if (h in e)
                return e[h];
            var f = b(h), g = 0 < a(f).parents(".nav_deep").length, l = a(f).hasClass("nav_pad_5") ? 1 : 0, i = 0;
            g && (i -= 7);
            c(f, function(b) {
                1 == b.nodeType && (b = a(b), g ? (i += b.hasClass("nav_pop_li") ? 28 - 2 * l : 0, i += b.hasClass("nav_first") ? -6 + l : 0, i += b.hasClass("nav_divider_before") ? 15 : 0) : (i += b.hasClass("nav_pop_li") ? 23 : 0, i += 
                b.hasClass("nav_divider_before") ? 10 : 0, i += b.hasClass("nav_first") ? -7 : 0), i += b.hasClass("nav_tag") ? 13 : 0)
            });
            return e[h] = i
        }
    });
    c.when("$popover", "$byID", "agent", "logEvent", "recordEv", "areaMapper", "flyout.NavButton", "flyout.SKIN", "flyout.computeFlyoutHeight", "flyout.initBrowsePromos", "config.responsiveGW", "config.sbd", "config.newFlyouts", "nav.inline", "flyout.content").run("ShopAll", function(a, b, d, e, h, f, g, l, i, j, k, n, m) {
        if (!m && b("nav-shop-all-button").length && b("nav_browse_flyout").length) {
            var s = function() {
            }, 
            u = a.AmazonPopover.mouseTracker, q = a.AmazonPopover.updateBacking, p, A, y, B = [], r, E, v, C, H, z, t, D, x = new g("nav-shop-all-button"), F = b("nav-shop-all-button"), w = k && (!d.touch || d.ie10touch);
            d.touch && a("#nav_cats .nav_cat a").each(function() {
                var b = a(this);
                b.replaceWith(b.text())
            });
            var G = function() {
                var g = c.getNow("initExposeSBD");
                g && g().deBorder(!0);
                f.disable("#nav_browse_flyout");
                j();
                var k = b("nav_browse_flyout"), l = b("nav_subcats"), g = b("nav_subcats_wrap"), m = b("nav_cats_wrap"), s = b("nav_cat_indicator");
                p ? k.css({height: i("nav_cats_wrap") + 
                    "px"}) : (y = m.outerWidth(), k.css({height: i("nav_cats_wrap") + "px",width: y + "px"}), g.css({display: "block"}), p = !0);
                var t = function() {
                    A && A();
                    A = null;
                    if (d.touch) {
                    ...
                    var c = "c", d = "c";
                    a && b && (a.x < b.x1 ? c = "l" : a.x > b.x2 && (c = "r"), a.y < b.y1 ? d = "t" : a.y > b.y2 && (d = "b"));
                    return d + c
                }, x = function(a, b) {
                    var c = 0, d = a.cursor, e = a.priorCursors[0] || {}, f = a.priorCursors[1] || {};
                    if (d.x == e.x && 2 > Math.abs(d.y - 
                    e.y) && d.x > f.x)
                        c = n.minor_delay;
                    else {
                        e = [d, e];
                        switch (z(d, b)) {
                            case "tl":
                                e.push({x: b.x1,y: b.y2}, {x: b.x2,y: b.y1});
                                break;
                            case "bl":
                                e.push({x: b.x1,y: b.y1}, {x: b.x2,y: b.y2});
                                break;
                            case "cl":
                                e.push({x: b.x1,y: b.y1}, {x: b.x1,y: b.y2});
                                break;
                            default:
                                e.push({x: 0,y: 0}, {x: 0,y: 0}), c = -1
                        }
                        0 === c && (c = (e[2].x - e[1].x) * (e[3].y - e[1].y) - (e[3].x - e[1].x) * (e[2].y - e[1].y), d = ((e[3].x - e[0].x) * (e[1].y - e[0].y) - (e[1].x - e[0].x) * (e[3].y - e[0].y)) / c, f = ((e[1].x - e[0].x) * (e[2].y - e[0].y) - (e[2].x - e[0].x) * (e[1].y - e[0].y)) / c, c = 0 < ((e[2].x - e[0].x) * 
                        (e[3].y - e[0].y) - (e[3].x - e[0].x) * (e[2].y - e[0].y)) / c && 0 < d && 0 < f ? n.major_delay : 0)
                    }
                    return c
                }, F = function(f) {
                    var g = !r, i = !1, j = b("nav_subcats_" + f), f = b("nav_cat_" + f), m = j.attr("data-nav-promo-id"), v = j.attr("data-nav-wt");
                    r && (b("nav_subcats_" + r).css({display: "none"}), b("nav_cat_" + r).removeClass("nav_active"));
                    clearTimeout(D);
                    if (m) {
                        var p = c.getNow("config.browsePromos", {}), z = {t: "sa",id: m};
                        p[m] && (z.bp = 1);
                    ...
                    v && h(v);
                    f.addClass("nav_active");
                    j.css({display: "block"});
                    s.css("top", 
                    f.position().top + parseInt(f.css("padding-top"), 10) + 1);
                    f = j.hasClass("nav_super_cat");
                    f != E && (f ? k.addClass("nav_super") : k.removeClass("nav_super"), i = !0, E = f);
                    g ? (A = function() {
                        q("navbar")
                    }, k.animate({width: l.outerWidth() + y + "px"}, {duration: "fast",complete: t})) : i && (g = function() {
                        k.css({width: l.outerWidth() + y + "px"});
                        q("navbar")
                    }, A ? A = g : g());
                    w && !d.ie6 && j.parents(".nav_exposed_skin").removeClass("nav_exposed_skin");
                    j.find(".nav_subcat_links").each(function() {
                        var b = a(this);
                        if (!b.data("nav-linestarts-marked")) {
                            b.data("nav-linestarts-marked", 
                            !0);
                            var c = 0;
                            b.find("li").each(function() {
                                var b = a(this), d = b.offset().top;
                                5 < Math.abs(d - c) && (b.addClass("nav_linestart"), c = d)
                            })
                        }
                    });
                    i = j.offset();
                    g = i.left;
                    i = i.top - n.target_slop;
                    f = g + j.outerWidth();
                    j = i + j.outerHeight() + n.target_slop;
                    return {x1: g,y1: i,x2: f,y2: j}
                };
                window._navbar.qaActivateCat = function(a) {
                    a = a || "0";
                    F(a);
                    r = a
                };
                a("#nav_cats li.nav_cat").each(function() {
                    var c = /^nav_cat_(.+)/.exec(this.id), d = c ? c[1] : "", c = a(this), e = c.offset(), c = u.add([[e.left, e.top, c.outerWidth(), c.outerHeight()]], {inside: !1,mouseEnter: function(a) {
                            clearTimeout(C);
                            b("nav_cat_" + d).addClass("nav_hover");
                            if (r !== d) {
                                var c = x(a, v);
                                if (r && 0 < c) {
                                    var e = function() {
                                        clearTimeout(C);
                                        var b = u.getCallbackArgs(), c = 0;
                                        H && H.x == b.cursor.x && H.y == b.cursor.y ? (c = "cc" == z(b.cursor, v) ? !1 : !0, c = c ? 0 : -1) : c = x(b, v);
                                        H = {x: b.cursor.x,y: b.cursor.y};
                                        0 < c ? r !== d && (C = setTimeout(e, c)) : -1 < c && (v = F(d, a), r = d)
                                    };
                                    C = setTimeout(e, c)
                                } else
                                    v = F(d, a), r = d
                            ...
                            b("nav_cat_" + d).removeClass("nav_hover");
                            return !0
                        }});
                    B.push(c)
                })
                ...
                r && (b("nav_subcats_" + r).css({display: "none"}), b("nav_cat_" + r).removeClass("nav_active"));
                r = null;
                b("nav_cat_indicator").css({top: ""});
                b("nav_browse_flyout").css({height: "",overflow: ""});
                f.enable();
                (a = c.getNow("initExposeSBD")) && a().deBorder(!1)
            };
            c.declare("initExposeSBD", function() {
                if (z)
                    return z;
                if (!w)
                    return z = {ok: function() {
                            return !1
                        },deBorder: s,initDeferredShow: s,deferredShow: s};
                var f = b("nav_exposed_anchor"), k = d.ie6 ? "" : "nav_exposed_skin", i = l(F, 0, k), 
                j = a("function" == typeof i ? i() : i), i = a('<div id="nav_exposed_skin"></div>').css({top: -8,left: F.offset().left - (d.ie6 ? 27 : 30)}).append(j).appendTo(f), n = b("nav_browse_flyout"), m = a('<div id="nav_exposed_cats"></div>').appendTo(a(".ap_content", i)), v = b("nav-shop-all-button").clone().attr({href: "javascript:void(0)",id: ""}).addClass("nav-shop-all-button nav-button-outer-open").appendTo("#nav-bar-inner").add(i), r = b("navbar").add(f), q = b("nav_cats_wrap"), p = new g("nav_exposed_skin"), A, t = !0, u, C, y, f = a.browser.msie && 
                (!document.documentMode || 8 > document.documentMode);
                z = {ok: function() {
                        return !0
                    },deBorder: function(a) {
                        a ? j.addClass("nav_pop_triggered") : j.removeClass("nav_pop_triggered")
                    },initDeferredShow: function() {
                        C || (C = s)
                    },deferredShow: function() {
                        C && (C(), C = null)
                    }};
                var D = {localContent: "#nav_browse_flyout",location: "top",locationAlign: "left",locationOffset: [30, f ? 33 : 32],skin: l(m, 0, k),showOnHover: !0,hoverShowDelay: 0,onShow: function() {
                        y || (y = s);
                        p.removeTrigger();
                        p.onShow();
                        G();
                ...
                c.when("protectExposeSBD", "$byID").run("exposeSBD", function(b, c) {
                    b(function(b) {
                        u = b;
                        b !== A && (b ? (e({t: "sa",id: "res-main"}), b = function() {
                            if (!1 !== u) {
                                x.removeTrigger();
                                r.addClass("nav_exposed_sbd");
                                var b = c("nav_browse_flyout");
                                b.hasClass("nav_deep") && r.addClass("nav_deep");
                                q.appendTo(m);
                                t && (a.browser.msie ? v.css("display", "block") : v.fadeIn(600), t = !1);
                                p.registerTrigger(D);
                                (b = b.attr("data-nav-wt")) && h(b);
                                A = !0
                            }
                        }, C ? C = b : b()) : (b = function() {
                            !0 !== u && (p.removeTrigger(), 
                            r.removeClass("nav_exposed_sbd"), q.appendTo(n), t && (v.css("display", "block"), t = !1), x.registerTrigger(K), A = !1)
                        }, y ? y = b : b()))
                    })
                });
                return z
            });
            var K = {localContent: "#nav_browse_flyout",locationAlign: "left",locationOffset: [d.ie6 ? 3 : 0, 0],skin: l(F, 0),onShow: function() {
                    var a = c.getNow("initExposeSBD");
                    a && a().initDeferredShow();
                    x.onShow();
            ...
            w || x.registerTrigger(K);
            c.declare("flyout.shopall")
        }
    });
    c.when("$", "$byID", "nav.inline", "flyout.content").build("flyout.notificationCount", function(a, b) {
        function d(a) {
            e = a;
            h && (0 >= e ? h.remove() : h.text(9 < e ? "9+" : e))
        }
        var e = parseInt(a("#nav-noti-wrapper .nav-noti-content").attr("data-noti-count") || "0", 10), h;
        d.count = function() {
            return e
        };
        ...
        c.when("page.ready").run("NotificationIcon", function() {
            if (!(0 >= e)) {
                var c = b("nav-signin-text"), g = a.trim(c.text()).match(/^(.*?)(\.*)$/);
                c.html(g[1] + '<span id="nav-noti-count-position"></span>' + 
                g[2]);
                var g = b("nav-noti-count-position").position().left + 10, l = b("nav-your-account"), i = l.outerWidth() - 5 - g, j = {};
                15 > i || 15 > g ? (j.right = 2, 0 < i && 10 >= i && c.append(Array(11).join("&nbsp;"))) : j.left = Math.round(g) + 1;
                h = a('<div id="nav-noti-count" class="nav-sprite">');
                h.css(j).appendTo(l);
                d(e)
            }
        });
    ...
    c.when("$", "$byID", "agent", "flyout.notificationCount", "config.dismissNotificationUrl", "config.newFlyouts", "flyout.content").build("flyout.notifications", function(a, b, c, e, h, f) {
        function g() {
            var c = l.height() - b("nav-noti-all").outerHeight(!0);
            i.each(function() {
                var b = a(this);
                b.attr("data-dismissed") || (b.position().top + b.outerHeight() > c ? b.addClass("nav-noti-overflow") : b.removeClass("nav-noti-overflow"))
            })
        }
        var l = b("nav-noti-wrapper").find(".nav-noti-content"), i = l.find(".nav-noti-item").not("#nav-noti-empty");
        return {width: f ? 200 : 180,exists: function() {
                return 0 < e.count() && 0 < l.length
            },getContent: function() {
                var f = l.get(0);
                f.parentNode.removeChild(f);
                c.touch ? i.addClass("nav-noti-touch") : i.hover(function() {
                    a(this).addClass("nav-noti-hover")
                }, function() {
                    a(this).removeClass("nav-noti-hover")
                });
                a(".nav-noti-x", l).click(function(c) {
                    c.preventDefault();
                    c = a(this);
                    a.ajax({url: h,type: "POST",data: {id: c.attr("data-noti-id")},cache: !1,timeout: 500});
                    c.css("visibility", "hidden").parent().attr("data-dismissed", "1").slideUp(400, function() {
                        e.decrement();
                        0 === e.count() && b("nav-noti-empty").fadeIn(300);
                        g()
                    })
                }).hover(function() {
                ...
                return l
            },onShow: g,event: 0 < e.count() ? "noti" : null}
    });
    c.when("$", "config.newFlyouts", "flyout.content").build("flyout.highConfidence", 
    function(a, b) {
        var c = a("#csr-hcb-wrapper .csr-hcb-content");
        return {width: b ? 250 : 229,exists: function() {
                return 0 < c.length
            },getContent: function() {
                var a = c.get(0);
                a.parentNode.removeChild(a);
    ...
    c.when("$", "$byID", "areaMapper", "agent", "logEvent", "flyout.NavButton", "flyout.SKIN", "ajax.flyoutDispatcher", "flyout.notifications", "flyout.highConfidence", "config.signOutText", "config.newFlyouts", "nav.inline", "flyout.content").run("YourAccount", function(a, 
    b, d, e, h, f, g, l, i, j, k, n) {
        if (!n) {
            var m;
            i.exists() ? m = i : j.exists() && (m = j);
            var i = b("nav-your-account"), s = b("nav_your_account_flyout"), u;
            if (i.length && s.length) {
                var q = new f("nav-your-account");
                a("#nav-noti-wrapper, #csr-hcb-wrapper");
                var p, b = b("nav-wishlist").length || b("nav-cart").length, f = 0;
                m && (f = m.width + 19, u = m.getContent(), s.css("margin-left", f).prepend(a('<div id="nav_ya_sidebar_wrapper"></div>').css({width: f - 15,left: -f}).append(u)));
                q.registerTrigger({localContent: "#nav_your_account_flyout",locationAlign: b ? 
                    "left" : "right",locationOffset: [f ? -f : e.ie6 ? b ? 3 : -3 : 0, 0],skin: g(i, b || !e.ie6 ? 0 : 7),onShow: function() {
                        q.onShow();
                        if (m && m.onShow)
                            m.onShow();
                        ...
                        p = window.setTimeout(function() {
                            var a = {t: "ya"};
                            m && m.event && (a[m.event] = 1);
                            h(a)
                        }, 750);
                        u && u.height(s.height());
                        l.loadDynamicMenusConditionally()
                    },onHide: function() {
                        clearTimeout(p);
                        d.enable();
                        q.onHide()
                    },followLink: !e.touch});
                k && a("#nav-item-signout").html(k);
                c.declare("flyout.youraccount")
            }
        }
    });
    c.when("$", "$byID", "areaMapper", "agent", 
    "logEvent", "triggerProceedToCheckout", "flyout.NavButton", "flyout.SKIN", "ajax.flyoutDispatcher", "recordEv", "config.newFlyouts", "nav.inline", "flyout.content").run("Cart", function(a, b, d, e, h, f, g, l, i, j, k) {
        if (!k && b("nav-cart").length && b("nav_cart_flyout").length) {
            var n = !1, m = new g("nav-cart"), b = b("nav-cart"), s;
            m.registerTrigger({localContent: "#nav_cart_flyout",locationAlign: "right",locationOffset: [e.ie6 ? -3 : 0, 0],skin: l(b, e.ie6 ? 7 : 0),onShow: function() {
                    m.onShow();
                    d.disable("#nav_cart_flyout");
                    s = window.setTimeout(function() {
                        h({t: "cart"});
                        !n && 0 < a(".nav_cart_item").length && (j(21381), n = !0)
                    }, 750);
                    i.loadDynamicMenusConditionally();
                    f()
                },onHide: function() {
                    clearTimeout(s);
                    d.enable();
            ...
            c.declare("flyout.cart")
        }
    });
    c.when("$", "$byID", "areaMapper", "agent", "logEvent", "flyout.NavButton", "flyout.SKIN", "ajax.flyoutDispatcher", "config.newFlyouts", "nav.inline", "flyout.content").run("WishList", function(a, b, d, e, h, f, g, l, i) {
        if (!i && b("nav-wishlist").length && b("nav_wishlist_flyout").length) {
            var j = new f("nav-wishlist"), b = b("nav-wishlist"), 
            k;
            j.registerTrigger({localContent: "#nav_wishlist_flyout",locationAlign: "right",locationOffset: [e.ie6 ? -3 : 0, 0],skin: g(b, e.ie6 ? 7 : 0),onShow: function() {
                    j.onShow();
                    ...
                    l.loadDynamicMenusConditionally();
                    var b = a("#nav_wishlist_flyout"), c = b.outerWidth();
                    b.css("width", c).find(".nav_pop_ul").css("width", c).addClass("nav_pop_ul_wrap")
                },onHide: function() {
                    clearTimeout(k);
                    a("#nav_wishlist_flyout").css("width", "").find(".nav_pop_ul").css("width", 
                    "").removeClass("nav_pop_ul_wrap");
                    d.enable();
                    j.onHide()
                },followLink: !e.touch});
            c.declare("flyout.wishlist")
        }
    });
    c.when("$", "$byID", "flyout.NavButton", "areaMapper", "logEvent", "agent", "config.newFlyouts", "nav.inline", "flyout.content").run("PrimeTooltip", function(a, b, c, e, h, f, g) {
        if (!g && !(0 < a("#nav-prime-ttt.nav-alt-tip").length || b("navbar").hasClass("nav-prime")) && b("nav-prime-ttt").length && b("nav-prime-tooltip").length) {
            var l = new c("nav-prime-ttt"), b = b("nav-prime-ttt"), i, c = a("#navbar").hasClass("nav-logo-large") || 
            0 < a("#navbar").find(".nav-size-large").length, a = a.browser.msie;
            b.css("padding-right", c ? "21px" : "25px");
            l.registerTrigger({localContent: "#nav-prime-tooltip",width: null,location: "right",locationAlign: "top",locationOffset: [a ? -3 : 0, c ? -30 : -23],onShow: function() {
                    l.onShow();
                    e.disable("#nav-prime-tooltip");
                    ...
                    e.enable();
                    l.onHide()
                },zIndex: 201,skin: '<div class="nav-tt-skin' + (c ? " nav-logo-large" : "") + '"><div class="nav-tt-border' + 
                (a ? "" : " nav-tt-box-shadow") + '"><div class="ap_content"></div></div><div class="nav-tt-beak"></div><div class="nav-tt-beak-2"></div></div>',followLink: !f.touch})
        }
    });
    c.when("$byID", "areaMapper", "logEvent", "agent", "config.dynamicMenuArgs", "config.newFlyouts", "flyout.NavButton", "flyout.SKIN", "flyout.primeAjax", "ajax.flyoutDispatcher", "nav.inline", "flyout.content").run("PrimeMenu", function(a, b, d, e, h, f, g, l, i, j) {
        if (!f && a("nav-your-prime").length && a("nav-prime-menu").length) {
            var k = new g("nav-your-prime"), 
            f = a("nav-your-prime"), n;
            i && a("nav-prime-menu").css({width: h.primeMenuWidth});
            k.registerTrigger({localContent: "#nav-prime-menu",locationAlign: "right",locationOffset: [e.ie6 ? -3 : 0, 0],skin: l(f, e.ie6 ? 7 : 0),onShow: function() {
                    k.onShow();
                    b.disable("#nav-prime-menu");
                    n = window.setTimeout(function() {
                        d({t: "prime"})
                    }, 750);
                    i && j.loadDynamicMenusConditionally()
                },onHide: function() {
                    clearTimeout(n);
                    b.enable();
                    k.onHide()
                },followLink: !e.touch});
            c.declare("flyout.prime")
        }
    });
    c.when("$", "$byID", "agent", "template", "config.newFlyouts", 
    "nav.inline").build("flyout.initBrowsePromos", function(a, b, d, e, h) {
        var f = !1;
        return function() {
            var g = c.getNow("config.browsePromos");
            g && !f && (f = !0, a("#nav_browse_flyout .nav_browse_subcat").each(function() {
                var c = a(this), f = c.attr("data-nav-promo-id");
                if (f) {
                    var j = g[f];
                    if (j)
                        if (j.promoType) {
                            "wide" == j.promoType && c.addClass("nav_super_cat");
                            var f = "nav_imgmap_" + f, k = d.ie6 ? 15 : 14, n = parseInt(j.vertOffset, 10) - k, m = parseInt(j.horizOffset, 10), s = (k = d.ie6 && /\.png$/i.test(j.image)) ? b("nav_trans_pixel").attr("src") : j.image, 
                            n = a("<img>").attr({src: s,alt: j.alt,useMap: "#" + f}).addClass("nav_browse_promo").css({bottom: n,right: m,width: j.width,height: j.height});
                            h && c.prepend(b(f));
                            c.prepend(n);
                            k && (n.get(0).style.filter = "progid:DXImageTransform.Microsoft.AlphaImageLoader(src='" + j.image + "',sizingMethod='scale')")
                        } else
                            c.prepend(e("#nav-tpl-asin-promo", j))
                }
            }))
        }
    });
    c.when("$", "agent", "config.flyoutURL", "btf.full").run("FlyoutContent", function(a, b, d) {
        window._navbar.flyoutContent = function(b) {
            var d = a("<div>").appendTo(document.body).hide().get(0), 
            f = "", g;
            for (g in b)
                b.hasOwnProperty(g) && (f += b[g]);
            d.innerHTML = f;
            c.declare("flyout.content")
        };
        d ? (b = document.createElement("script"), b.setAttribute("type", "text/javascript"), b.setAttribute("src", d), (document.head || document.getElementsByTagName("head")[0]).appendChild(b)) : c.declare("flyout.content")
    });
    c.when("$", "$F", "$byID", "config.enableDynamicMenus", "config.dynamicMenuUrl", "config.dynamicMenuArgs", "config.ajaxProximity", "config.newFlyouts", "flyout.primeAjax", "AFDFactory", "AJAXFlyoutRenderer", "nav.inline").run("DynamicAjaxMenu", 
    function(a, b, d, e, h, f, g, l, i, j, k) {
        var b = a("#nav_wishlist", "#nav_cart"), n = d("nav_wishlist_flyout"), m = d("nav_cart_flyout"), g = a(n).add(m), s = a(".nav_dynamic", n), u = a(".nav_dynamic", m);
        if (i)
            var q = d("nav-prime-menu"), g = g.add(q), p = a(".nav_dynamic", q), b = b.add("#nav-your-prime");
        d = new j({url: h,isLoaded: !e,triggerElements: b,flyoutElements: g,data: f,successFunc: function(b) {
                b = a.extend({cartDataStatus: !1,cartCount: 0,cart: [],wishlistDataStatus: !1,wishlist: [],primeMenu: null}, b);
                b.cartDataStatus && c.getNow("api.setCartCount", 
                function() {
                })(b.cartCount);
                i && k({status: !!b.primeMenu,isEmpty: !b.primeMenu,parentElem: q,rawHTML: b.primeMenu,templateID: null,contentElem: p,data: b});
                k({status: b.cartDataStatus,isEmpty: 0 === b.cart.length,parentElem: m,rawHTML: null,templateID: "#nav-tpl-cart",contentElem: u,data: b});
                k({status: b.wishlistDataStatus,isEmpty: 0 === b.wishlist.length,parentElem: n,rawHTML: null,templateID: "#nav-tpl-wishlist",contentElem: s,data: b});
                return b.cartDataStatus && b.wishlistDataStatus
            },prefetchEventName: "event.prefetch"});
        c.declare("ajax.flyoutDispatcher", d)
    });
    c.when("$", "$byID", "$F", "agent", "logEvent", "recordEv", "isAUI", "flyout.create", "flyout.initBrowsePromos", "flyout.onClickDebug", "flyout.Engine", "flyout.Group", "MouseTracker", "flyout.SloppyTrigger", "flyout.LockTrigger", "flyout.setupTriggers", "config.newFlyouts", "config.responsiveGW", "dismissTooltip", "nav.inline", "flyout.content").run("NewShopAll", function(a, b, d, e, h, f, g, l, i, j, k, n, m, s, u, q, p, A, y) {
        if (p) {
            var B, r = l("ShopAll", {button: b("nav-shop-all-button"),content: b("nav_browse_flyout"),
                noClick: !0,align: "left",classes: g() ? ["nav-flyout", "nav-flyout-sa"] : [],blur: function() {
                    return !B
                }}), E = function(a, b) {
                var c = {t: "sa",id: a};
                b && (c.bp = 1);
            ...
            r.onShow(function() {
                var a = r.elem(), b = a.innerHeight();
                a.find(".nav_subcats_div, .nav_browse_subcat").css("height", b)
            });
            var C = new n, H = !1;
            r.onRender(function(g) {
                g.addClass("nav-flyout-content");
                var h = function(b) {
                    a(b).find("#nav_cats .nav_cat a").each(function() {
                        var b = a(this);
                        b.replaceWith(b.text())
                    })
                };
                e.touch ? h() : j(h);
                var n = g.hasClass("nav_deep");
                n && g.addClass("nav_deep_css3");
                var l = new s;
                r.onHide(function() {
                    l.reset()
                });
                a(g).find("li.nav_cat").each(function() {
                    var b = a(this), h = /^nav_cat_(.+)/.exec(b.attr("id"));
                    if (h) {
                        var j = g.find("#nav_subcats_" + h[1]);
                        if (0 !== j.length) {
                            var m = new k({content: j});
                            C.register(m, j);
                            q(m, function(a) {
                                l.register(b, j, a)
                            }, function(a) {
                                b.bind(e.touch ? "mousedown" : "click", d.bind(a).on(a.show))
                            });
                            b.hover(function() {
                                b.addClass("nav_hover")
                            }, function() {
                                b.removeClass("nav_hover")
                            });
                            m.onShow(function() {
                                b.addClass("nav_active")
                            });
                            m.onHide(function() {
                                b.removeClass("nav_active")
                            });
                            m.onShow(function() {
                                g.find("#nav_cat_indicator").css({display: "block",top: b.position().top + parseInt(b.css("padding-top"), 10) + 1});
                                window.$Nav.getNow("iss", {blur: function() {
                                    }}).blur();
                                y()
                            });
                            m.onRender(i);
                            m.onInteract(d.memoize().on(function() {
                                var a = m.elem().attr("data-nav-promo-id");
                                if (a) {
                                    var d = c.getNow("config.browsePromos", {});
                                    E(a, !!d[a])
                                }
                                (a = b.attr("data-nav-wt")) && f(a)
                            }));
                            if (n) {
                                var v = !1;
                                m.onShow(function() {
                                    v || 
                                    (v = !0, m.elem().find(".nav_subcat_links").each(function() {
                                        var b = 0;
                                        a(this).find("li").each(function() {
                                            var c = a(this), d = c.offset().top;
                                            5 < Math.abs(d - b) && (c.addClass("nav_linestart"), b = d)
                                        })
                                    }))
                                })
                            }
                ...
                C.onShow(function(a, b) {
                    var c = g.find("#nav_subcats_wrap"), d = parseInt(c.css("left"), 10) - (parseInt(g.css("padding-left") || "0", 10) + parseInt(g.css("padding-right") || "0", 10)), e = b.outerWidth();
                    H ? (g.css("width", d + e), r.resized()) : c.css({width: 0,overflow: "hidden"}).animate({width: e}, {duration: 200,step: function(a) {
                            g.css("width", 
                            d + a)
                        },complete: function() {
                            r.resized();
                ...
                r.onHideRequest(function() {
                    r.elem().css({width: ""});
                    H = !1;
                    C.hide();
                    l.reset();
                    b("nav_cat_indicator").hide()
                })
            });
            c.when("protectExposeSBD").run("exposeSBD", function(b) {
                b(function(b) {
                    var c = a("#navbar:not(.nav-aui) #nav-shop-all-button .nav-down-arrow");
                    (B = b) ? (r.show(), r.lock(), E("res-main"), c.hide()) : (r.hide(), r.unlock(), c.show())
                })
            });
            r.onInteract(function() {
                B || E("main")
            });
            r.onShow(function() {
                B || i()
            });
            c.declare("initExposeSBD", 
            function() {
                return {ok: function() {
                        return A && (!e.touch || e.ie10touch)
                    },deBorder: function() {
                    },initDeferredShow: function() {
                    },deferredShow: function() {
                    }}
    ...
    c.when("$", "$byID", "isAUI", "flyout.create", "flyout.position", "flyout.notifications", "flyout.highConfidence", "config.signOutText", "config.newFlyouts", "ajax.flyoutDispatcher", "nav.inline", "flyout.content").run("NewYourAccount", function(a, b, d, e, h, f, g, l, i, j) {
        if (i) {
            var k;
            f.exists() ? k = f : g.exists() && (k = g);
            f = {t: "ya"};
            k && k.event && 
            (f[k.event] = 1);
            d = e("yourAccount", {button: b("nav-your-account"),content: b("nav_your_account_flyout"),align: 0 === b("nav-wishlist").length && 0 === b("nav-cart").length ? "right" : k ? h("bottom left").to("top left", {x: -k.width}) : "left",event: f,ajaxFlyoutDispatcher: j,classes: d() ? ["nav-flyout", "nav-flyout-ya"] : []});
            if (k)
                d.onShow(function(a) {
                    b("nav_ya_sidebar_wrapper").height(a.elem().innerHeight());
                    k.onShow();
                    a.elem().find(".nav-arrow,.nav-arrow-inner").css("margin-left", k.width)
                });
            d.onRender(function(c) {
                l && b("nav-item-signout").html(l);
                if (k) {
                    var d = a('<div id="nav_ya_sidebar_wrapper">').css("width", k.width).append(k.getContent()), e = c.find(".nav-flyout-borders");
                    0 === e.length && (e = c);
                    e.css("padding-left", k.width + 15).prepend(d)
                }
            });
            c.declare("flyout.youraccount")
        }
    });
    c.when("$byID", "isAUI", "flyout.create", "triggerProceedToCheckout", "config.newFlyouts", "ajax.flyoutDispatcher", "nav.inline", "flyout.content").run("NewCart", function(a, b, d, e, h, f) {
        h && (d("cart", {button: a("nav-cart"),content: a("nav_cart_flyout"),align: "right",event: "cart",ajaxFlyoutDispatcher: f,
            classes: b() ? ["nav-flyout", "nav-flyout-cart"] : []}).onShow(e), c.declare("flyout.cart"))
    });
    c.when("$byID", "isAUI", "flyout.create", "agent", "config.newFlyouts", "ajax.flyoutDispatcher", "nav.inline", "flyout.content").run("NewWishList", function(a, b, d, e, h, f) {
        if (h) {
            var g = d("wishlist", {button: a("nav-wishlist"),content: a("nav_wishlist_flyout"),align: "right",event: "wishlist",ajaxFlyoutDispatcher: f,classes: b() ? ["nav-flyout", "nav-flyout-wl"] : []});
            g.onShow(function() {
                var a = Math.min(g.elem().outerWidth(), 350);
                g.elem().css("width", 
                a)
            });
            g.onHide(function() {
    ...
    c.when("$", "$byID", "flyout.create", "flyout.position", "config.newFlyouts", "nav.inline", "flyout.content").run("NewPrimeTooltip", function(a, b, c, e, h) {
        function f(a, b) {
        ...
        if (h && !(0 < a("#nav-prime-ttt.nav-alt-tip").length || b("navbar").hasClass("nav-prime"))) {
            var g = a("#navbar").hasClass("nav-logo-large") || 0 < b("navbar").find(".nav-size-large").length, 
            a = e("middle right", {x: 10,y: -2}).to("middle left", g ? {y: -3} : {y: 4});
            c("primeTT", {button: b("nav-prime-ttt"),content: b("nav-prime-tooltip"),align: a,event: "prime-tt",css: {zIndex: 201},markOpen: !1,skin: function(a) {
                    return f("nav-tt-skin" + (g ? " nav-logo-large" : ""), [f("nav-tt-border", [a]), f("nav-tt-beak"), f("nav-tt-beak-2")])
                }})
        }
    });
    c.when("$byID", "agent", "config.dynamicMenuArgs", "config.newFlyouts", "isAUI", "flyout.create", "flyout.primeAjax", "ajax.flyoutDispatcher", "nav.inline", "flyout.content").run("NewPrimeMenu", 
    function(a, b, d, e, h, f, g, l) {
        e && (f("prime", {button: a("nav-your-prime"),content: a("nav-prime-menu"),align: "right",event: "prime",ajaxFlyoutDispatcher: g ? l : void 0,classes: h() ? ["nav-flyout", "nav-flyout-prime"] : []}), g && a("nav-prime-menu").css({width: d.primeMenuWidth + (b.ie && b.quirks ? 40 : 0)}), c.declare("flyout.prime"))
    })
})(window.$Nav);
window.$Nav.when("$").build("subnav.getFlyouts", function(c) {
    return function(a) {
        return a ? c(".nav_subnav_flyout_container", a) : c(".nav_subnav_flyout_container")
    }
});
window.$Nav.when("$", "subnav.buildFlyouts").run("SubnavFlyouts", function(c, a) {
    a();
    c(".nav-subnav-down-arrow").show();
    window.$Nav.declare("flyout.subnavs")
});
window.$Nav.when("$", "$F", "$byID", "config.ajaxProximity", "config.newFlyouts", "flyout.primeAjax", "first", "AFDFactory", "AJAXFlyoutRenderer", "subnav.buildAJAXData", "subnav.getFlyouts", "nav.inline").run("SubnavFlyoutsAjaxMenu", function(c, a, b, d, e, h, f, g, l, i, j) {
    var a = window.$Nav.getNow("config.subnavFlyoutUrl", "#"), k = j(), d = c(".nav-subnav-flyout-menu"), g = new g({url: a,triggerElements: d,flyoutElements: k,data: i(),successFunc: function(a) {
            k = j();
            a = c.extend({subnavFlyoutsDataStatus: !1,subnavFlyouts: []}, a);
            if (b("nav-subnav").attr("data-category") !== 
            a.subnavCategory)
                return !1;
            for (var d = 0; d < a.subnavFlyouts.length; d++) {
                var e = f(k, "id", "nav_subnav_flyout_" + (a.subnavFlyouts[d].position || d)), g = c(".nav_dynamic", e);
                l({status: a.subnavFlyoutsDataStatus,isEmpty: !1,parentElem: e,rawHTML: null,templateID: "#nav-tpl-subnav",contentElem: g,data: a.subnavFlyouts[d]});
                (e = a.subnavFlyouts[d].flyoutWidth) && g.css({width: e})
            }
            return a.subnavFlyoutsDataStatus
        },prefetchEventName: "event.subnavFlyoutPrefetch"});
    window.$Nav.declare("ajax.subnavFlyoutDispatcher", g)
});
window.$Nav.when("$").build("subnav.buildAJAXData", function(c) {
    return function() {
        var a = c("#nav-subnav"), b = "", d = "";
        c("a[data-slots]", a).each(function() {
            var a = c(this);
            b += a.attr("data-slots") + ";";
            d += (a.attr("data-handler") || "default") + ";"
        });
        return {subnavCategory: a.attr("data-category"),navslots: b,navhandlers: d}
    }
});
window.$Nav.when("$", "$byID", "isAUI", "flyout.create", "flyout.position", "config.newFlyouts", "ajax.subnavFlyoutDispatcher", "agent", "subnav.getFlyouts", "nav.inline", "flyout.content").build("subnav.buildFlyouts", function(c, a, b, d, e, h, f, g, l) {
    return function() {
        var e = l();
        if (h && 0 !== e.length) {
            var j = b() ? ["nav-flyout", "nav-flyout-subnav"] : [];
            0 === a("nav-subnav-container").length ? j.push("nav-flyout-subnav-beaconstyle") : j.push("nav-flyout-subnav-auistyle");
            g.ie && j.push("nav-flyout-subnav-ie");
            e.each(function(a) {
                var b = 
                c(this), e = b.parent().siblings("a"), h = e.attr("data-flyout-full-width"), i = e.attr("data-event");
                d("subnavFlyout" + a, {button: e,content: b,align: "left",fullWidth: h,event: i,ajaxFlyoutDispatcher: f,classes: j}).onRender(function(a) {
                    var d = 5;
                    g.ie10 && (d = 7);
                    g.opera && (d = 8);
                    var e = c(".nav-arrow, .nav-arrow-inner", a), f = c('.nav-subnav-flyout-menu[data-flyoutid="' + a.attr("id") + '"]'), a = f.width();
                    h ? (f = c("span", f).offset(), e.css("left", f.left - d + "px")) : e.css("left", a - (d - 8) + "px");
                    b.css("min-width", a + "px")
                })
            })
        }
    }
});
window.$Nav.when("$", "$F", "config.newFlyouts", "config.ajaxProximity", "Proximity", "util.parseQueryString").build("AFDFactory", function(c, a, b, d, e, h) {
    return function(a) {
        var g = !1, l = !1, i = !1, j, a = c.extend({retryTimeout: 5E3,errorFunc: function() {
            },successFunc: function() {
                return !0
            },completeFunc: function() {
            }}, a), k = {loadDynamicMenusConditionally: function() {
                g || (g = !0, k.dispatch());
                i || (i = !0, window.$Nav.declare(a.prefetchEventName));
                return !0
            },attachTriggers: function() {
                if (a.triggerElements && 0 < a.triggerElements.length)
                    if (b)
                        j && 
                        j.unbind(), j = e.onEnter(a.triggerElements, d || [0, 0, 0, 0], k.loadDynamicMenusConditionally);
                    else {
                        var g = [], h = [], i = [], l = [];
                        a.triggerElements.each(function(a, b) {
                            var b = c(b), d = b.offset();
                            g.push(d.left);
                            i.push(d.top);
                            h.push(d.left + b.width());
                            l.push(d.top + b.height())
                        });
                        var q = d || [0, 0, 0, 0], p = Math.min.apply(Math, g) - q[3], A = Math.min.apply(Math, i) - q[0], y = Math.max.apply(Math, h) + q[1] - p, q = Math.max.apply(Math, l) + q[2] - A;
                        c.AmazonPopover.mouseTracker.add([[p, A, y, q]], {inside: !1,mouseEnter: k.loadDynamicMenusConditionally,mouseLeave: function() {
                                return !0
                            }})
                    }
            },
            allowRetry: function(b) {
                var c = function() {
                    g = a.isLoaded = !1
                };
                b ? setTimeout(c, b) : c()
                ...
                window.$Nav.when("flyout.content").run("AFDFactory.dispatch", function() {
                    if (!a.isLoaded && !l) {
                        l = !0;
                        window.$Nav.getNow("preloadSpinner", function() {
                        })();
                        a.flyoutElements.addClass("nav-ajax-loading").removeClass("nav-ajax-error nav-empty");
                        a.flyoutElements.find(".nav-ajax-success").hide();
                        c.AmazonPopover.updateBacking("navbar");
                        var b = h(a.data);
                        c.ajax({url: a.url,data: b,dataType: "json",cache: !1,timeout: 1E4,complete: function() {
                                a.flyoutElements.removeClass("nav-ajax-loading");
                                c.AmazonPopover.updateBacking("navbar");
                                a.completeFunc();
                                l = !1
                            },error: function() {
                                a.flyoutElements.addClass("nav-ajax-error");
                                k.allowRetry(a.retryTimeout);
                                a.errorFunc()
                            },success: function(b) {
                                a.isLoaded = a.successFunc(b);
                                a.isLoaded || k.allowRetry(a.retryTimeout)
                            }})
                    }
                })
            }};
        a.url && window.$Nav.when("page.ready").run("AFDFactory.pageReadyAttachTriggers", k.attachTriggers);
        return k
    }
});
window.$Nav.when("template").build("AJAXFlyoutRenderer", function(c) {
    return function(a) {
        a.status ? (a.isEmpty ? a.parentElem.addClass("nav-empty").removeClass("nav-full") : a.parentElem.addClass("nav-full").removeClass("nav-empty"), a.contentElem.html(a.rawHTML || c(a.templateID, a.data)), a.parentElem.find(".nav-ajax-success").show()) : a.parentElem.addClass("nav-ajax-error")
    }
});
window.$Nav.build("util.parseQueryString", function() {
    return function(c) {
        c = c || {};
...
window.$Nav.when("$", "onOptionClick", "$F", "byID", "$byID", "btf.lite", "nav.inline").run("SearchDropdown", function(c, a, b, d, e) {
    function h() {
        var a = u.width();
        s.width() < a && s.width(a)
    }
    function f() {
        if (B.isFocus) {
            E.addClass("focus nav-focus").removeClass("active nav-active");
            ...
            B.isHover || B.searchFocus || B.searchHover ? E.addClass("active nav-active").removeClass("focus nav-focus") : E.removeClass("active focus nav-active nav-focus")
    }
    function g() {
        var a = c(q).width();
        return 195 < a || 100 < a && 400 >= A.outerWidth() ? !0 : !1
    }
    function l() {
        c(u).add(q).css({width: "auto"});
        c(q).css({overflow: "visible"});
        g() && c(q).css({width: 100,overflow: "hidden"});
        setTimeout(function() {
            A.css({"padding-left": u.width()});
            c.browser.msie || p.css("padding-right", parseInt(p.css("padding-right"), 10) ? 0 : 1);
            h()
        }, 1)
    }
    function i() {
        var a;
        a: {
            var b = m.children.length;
            for (a = 0; a < b; a++)
                if (m.children[a].selected) {
                    a = c(m.children[a]);
                    break a
                }
            a = s.children("option:selected")
        }
        b = a.val();
        b !== r && (a = r || b !== y ? a.html() : q.innerHTML, r = b, q.innerHTML !== a && (q.innerHTML = a, l()));
        f()
    }
    function j() {
        var a = window.$Nav.getNow("iss");
        a && (B.isFocus = !1, a.focus())
    }
    function k(a) {
        13 === a.which && j();
        9 !== a.which && 16 !== a.which && i()
    }
    function n(a, b) {
        return function() {
            B[a] = b;
    ...
    var m = d("searchDropdownBox"), s = c(m), u = e("nav-search-in"), q = d("nav-search-in-content"), p = e("twotabsearchtextbox"), A;
    a: {
        d = p;
        for (e = p.parent(); d.length; ) {
    ...
    var y = q.getAttribute && 
    q.getAttribute("data-value") || c(q).attr("data-value"), B = {isHover: !1,isFocus: !1,searchFocus: !1,searchHover: !1}, r = null;
    if (c.browser.msie) {
        if (7 > parseFloat(c.browser.version)) {
    ...
    var E = u.add(".nav-searchbar-inner");
    window.$Nav.declare("refreshDropDownFacade", i);
    u.get(0).className += " nav-facade-active";
    i();
    s.change(i).keyup(k).focus(n("isFocus", !0)).blur(n("isFocus", !1)).hover(n("isHover", !0), n("isHover", !1));
    window.$Nav.when("dismissTooltip").run("SddHideTooltip", 
    function(a) {
        s.focus(a)
    });
    a(s, function() {
    ...
    c(window).resize(b.throttle(150).on(l));
    window.$Nav.when("page.ready").run("FixSearchDropdown", function() {
        (4 <= c(q).outerWidth(!0) - u.outerWidth() || g()) && l();
        h();
        s.css({top: Math.max(0, (u.outerHeight() - s.outerHeight()) / 2)})
    });
    window.$Nav.when("iss").run("SddIss", function(a) {
        a.keydown(function(a) {
            setTimeout(function() {
                k(a)
            }, 10)
        });
        window.$Nav.when("dismissTooltip").run("SearchTypeHideTooltip", function(b) {
            a.keydown(b)
        });
        a.onFocus(n("searchFocus", !0));
        a.onBlur(n("searchFocus", 
        !1));
        a.onBlur(i);
        if (a.onSearchBoxHover)
            a.onSearchBoxHover(n("searchHover", !0), n("searchHover", !1))
    })
});
window.$Nav.when("$").build("Keycode", function(c) {
    function a(a) {
        this.evt = a;
        this.code = a.keyCode
    }
    a.prototype.isAugmented = function() {
        return this.evt.altKey || this.evt.ctrlKey || this.evt.metaKey
    };
    a.prototype.isAugmentor = function() {
        return 0 <= c.inArray(this.code, [0, 16, 20, 17, 18, 224, 91, 93])
    };
    a.prototype.isTextFieldControl = function() {
        return 0 <= c.inArray(this.code, [8, 9, 13, 32, 35, 36, 37, 38, 39, 40, 45, 46])
    };
    a.prototype.isControl = function() {
        return 46 >= this.code || 91 <= this.code && 95 >= this.code || 112 <= this.code && 145 >= this.code ? 
        !0 : !1
    };
    a.prototype.isTab = function() {
        return 9 === this.code
    };
    a.prototype.isEnter = function() {
        return 13 === this.code
    };
    a.prototype.isBackspace = function() {
        return 8 === this.code
    };
    return a
});
window.$Nav.when("$", "agent", "iss", "Keycode", "config.autoFocus", "nav.inline").run("autoFocus", function(c, a, b, d, e) {
    function h() {
        return c(document).scrollTop() <= c("#nav-iss-attach").offset().top && 1 > c(document.activeElement).filter("input,select,textarea").size() && "" === (window.getSelection ? window.getSelection().toString() : document.selection ? document.selection.createRange().text : "")
    }
    if (e && !a.touch) {
        var f = !1;
        h() && (b.focus(), f = !0);
        b.keydown(function(a) {
            ...
            if (!a.isAugmentor()) {
                var c = a.isControl();
                a.isAugmented() || (f ? h() ? c && b.blur() : b.blur() : c && (a.isTextFieldControl() ? "" === b.keyword() && (!a.isTab() && !a.isEnter() && !a.isBackspace()) && b.blur() : b.blur()));
                f = !1
            }
        });
        c(document).keydown(function(a) {
            a = new d(a);
            h() && (a.isControl() || !a.isAugmentor() && !a.isAugmented() && b.focus())
        })
    }
});
window.$Nav.when("$", "byID", "agent", "api.publish", "config.lightningDeals", "ajax.subnavFlyoutDispatcher", "subnav.buildFlyouts", "subnav.buildAJAXData", "subnav.getFlyouts").run("UpdateAPI", function(c, a, b, d, e, h, f, g, l) {
    function i(b) {
        if (b instanceof Object) {
            var d = window.$Nav.getNow("ajax.flyoutDispatcher");
            d && d.allowRetry();
            var d = [], i = [];
            if (b.catsubnav)
                try {
                    var s = c("#nav-subnav");
                    if (0 < s.length) {
                        var u = b.catsubnav.digest || "";
                        if (!u || u !== s.attr("data-digest")) {
                            var q = 0, p = [], A = b.catsubnav.categoryKey, y = b.catsubnav.category;
                            if (y && "link" == y.type) {
                                var B = c("li.nav-category-button:first", s);
                                if (0 === B.length)
                                    throw "category-1";
                                var r = y.data;
                                if (!r.href || !r.text)
                                    throw "category-2";
                                var E = B.clone(), v = c("a:first", E);
                                if (0 === v.length)
                                    throw "category-3";
                                v.attr("href", r.href).html(r.text);
                                p.push(E.get(0));
                                q += 1
                            }
                            var C = b.catsubnav.subnav;
                            if (C && "linkSequence" == C.type) {
                                var H = c("li.nav-subnav-item", s).slice(-1);
                                if (0 === H.length)
                                    throw "subnav-1";
                                for (var z = 0; z < C.data.length; z++) {
                                    var t = C.data[z];
                                    if (!t.href || !t.text)
                                        throw "subnav-2";
                                    var D = H.clone(), 
                                    ...
                                    if (0 === x.length)
                                        throw "subnav-3";
                                    x.attr("href", t.href).html(t.text);
                                    D.removeClass("nav-category-button nav-subnav-item-float-right nav-subnav-item-border-right");
                                    D.find("div").remove();
                                    x.find("span").remove();
                                    t.floatRight && D.addClass("nav-subnav-item-float-right");
                                    t.flyoutFullWidth && x.attr("data-flyout-full-width", "1");
                                    t.rightSpacer && D.addClass("nav-subnav-item-border-right");
                                    if (t.amabotData)
                                        for (var F in t.amabotData)
                                            t.amabotData.hasOwnProperty(F) && x.attr("data-" + F, t.amabotData[F]);
                                    t.flyout && (x.addClass("nav_a nav-subnav-flyout-menu"), x.attr("data-flyoutid", t.flyoutID), x.append('<span class="nav-subnav-down-arrow"></span>'), D.append(t.flyoutContainerDiv));
                                    p.push(D.get(0))
                                }
                                1 < p.length && (q += 1)
                            }
                            var w = c("#navbar"), G = c("#nav-subnav-container");
                            if (0 === w.length)
                                throw "catsubnav-1";
                            2 == q ? (d.push(function() {
                                l().remove();
                                s.empty().append(p).css("display", "").attr("data-digest", u).attr("data-category", A);
                                w.addClass("nav-subnav");
                                0 < G.length && G.css("display", "")
                            }), d.push(function() {
                                var a = l(p);
                                if (0 < a.length) {
                                    var b = c(".nav-subnav-down-arrow"), d = g();
                                    h.reload(b, a, d);
                                    f()
                                }
                            })) : 0 === q && d.push(function() {
                                w.removeClass("nav-subnav");
                                s.css("display", "none").attr("data-digest", "0");
                                0 < G.length && G.css("display", "none")
                            })
                        }
                    }
                } catch (N) {
                    return j = "navbar.update() error: " + N, !1
                }
            if (b.cart)
                try {
                    var K = b.cart;
                    if ("countPlusLD" == K.type || "count" == K.type) {
                        if (!K.data)
                            throw "cart-1";
                        var I = K.data.count + "";
                        if (!I.match(/^(|0|[1-9][0-9]*|99\+)$/))
                            throw "cart-2";
                        var $ = c("#nav-cart-count, #nav_cart_flyout .nav-cart-count");
                        0 < 
                        $.length && d.push(function() {
                            var b = "nav-cart-0";
                            "99+" == I ? b = "nav-cart-100" : 99 < I ? (I = "99+", b = "nav-cart-100") : 19 < I ? b = "nav-cart-20" : 9 < I && (b = "nav-cart-10");
                            $.removeClass("nav-cart-0 nav-cart-10 nav-cart-20 nav-cart-100").addClass(b).html(I);
                            0 === I && a("nav-cart-zero") ? (c("#nav-cart-one, #nav-cart-many").hide(), c("#nav-cart-zero").show()) : 1 == I ? (c("#nav-cart-zero, #nav-cart-many").hide(), c("#nav-cart-one").show()) : (c("#nav-cart-zero, #nav-cart-one").hide(), c("#nav-cart-many").show())
                        });
                        var aa = K.data.LDData;
                        aa && d.push(function() {
                            e = aa
                        })
                    }
                } catch (ha) {
                    return j = "navbar.update() error: " + ha, !1
                }
            if (b.searchbar)
                try {
                    var O = b.searchbar;
                    if ("searchbar" == O.type) {
                        if (!O.data)
                            throw "searchbar-1";
                        var P = O.data.options;
                        if (!P || 0 === P.length)
                            throw "searchbar-2";
                        var L = O.data["nav-metadata"] || {}, J = c("#searchDropdownBox");
                        0 === J.length && (J = c("#navSearchDropdown select:first"));
                        if (0 === J.length)
                            throw "searchbar-3";
                        !L.digest || L.digest !== J.attr("data-nav-digest") ? d.push(function() {
                            J.blur().empty();
                            for (var a = 0; a < P.length; a++) {
                                var b = P[a], 
                                d = b._display || "";
                                delete b._display;
                                c("<option></option>").html(d).attr(b).appendTo(J)
                            }
                            J.attr("data-nav-digest", L.digest || "").attr("data-nav-selected", L.selected || 0);
                            window.$Nav.getNow("refreshDropDownFacade", function() {
                            })()
                        }) : L.selected != J.attr("data-nav-selected") && d.push(function() {
                            J.attr("data-nav-selected", L.selected).get(0).selectedIndex = L.selected;
                            window.$Nav.getNow("refreshDropDownFacade", function() {
                            })()
                        })
                    }
                } catch (ka) {
                    return j = "navbar.update() error: " + ka, !1
                }
            if (b.primeBadge)
                try {
                    var ba = b.primeBadge.isPrime;
                    if ("boolean" == ba.type) {
                        w = c("#navbar");
                        if (0 === w.length)
                            throw "primeBadge-1";
                        d.push(function() {
                            ba.data ? w.addClass("nav-prime") : w.removeClass("nav-prime")
                        })
                    }
                    var T = b.primeBadge.homeUrl;
                    if ("html" == T.type) {
                        var ca = c("#nav-logo");
                        if (0 === ca.length)
                            throw "primeBadge-2";
                        T.data && d.push(function() {
                            ca.attr("href", T.data)
                        })
                    }
                } catch (ia) {
                    return j = "navbar.update() error: " + ia, !1
                }
            if (b.swmSlot)
                try {
                    var Q = b.swmSlot.swmContent;
                    if (Q && "html" == Q.type) {
                        var da = c("#navSwmSlot");
                        if (0 === da.length)
                            throw "swmContent-1";
                    ...
                    b.swmSlot.height;
                    if (R && "html" == R.type) {
                        var ea = c("#welcomeRowTable");
                        if (0 === ea.length)
                            throw "swmSlotHeight-1";
                        d.push(function() {
                            ea.css("height", R.data || "");
                            var a = /-(small|large)$/, b = c("#navbar");
                            c(b.attr("class").split(/\s+/)).filter(function() {
                                return a.test(this)
                            }).each(function() {
                                var c = 40 < parseInt(R.data || 0, 10), c = this.replace(a, c ? "-large" : "-small");
                                b.removeClass(this).addClass(c)
                            })
                        })
                    }
                    var U = b.swmSlot.style;
                    if (U && "html" == U.type) {
                        var fa = c("#nav-ad-background-style");
                        if (0 === fa.length)
                            throw "swmSlotStyle-1";
                        d.push(function() {
                            fa.attr("style", U.data || "")
                        })
                    }
                } catch (ja) {
                    return j = "navbar.update() error: " + ja, !1
                }
            if (b.signInText)
                try {
                    var V = b.signInText.customerName, W = b.signInText.greetingText;
                    if ("html" == V.type && W.type) {
                        var X = c("#nav-signin-title");
                        if (0 === X.length)
                            throw "signInText-1";
                        var M = X.attr("data-template");
                        M && (W.data && V.data) && (M = M.replace("{helloText}", W.data), M = M.replace("{signInText}", V.data), d.push(function() {
                            X.html(M)
                        }))
                    }
                } catch (la) {
                    return j = "navbar.update() error: " + la, !1
                }
            if (b.yourAccountLink)
                try {
                    var Y = 
                    b.yourAccountLink;
                    if ("html" == Y.type) {
                        var ga = c("#nav-your-account");
                        if (0 === ga.length)
                            throw "your-account-1";
                        Y.data && d.push(function() {
                            ga.attr("href", Y.data)
                        })
                    }
                } catch (ma) {
                    return j = "navbar.update() error: " + ma, !1
                }
            if (b.crossShop)
                try {
                    var S = b.crossShop;
                    if ("html" == S.type) {
                        var Z = c("#nav-your-amazon");
                        if (0 === Z.length)
                            throw "yourAmazonText-1";
                        d.push(function() {
                            S.data && Z.text() != S.data && Z.html(S.data)
                        })
                    }
                    i.push(function() {
                        c("#nav-cross-shop-links").css("display", "")
                    })
                } catch (na) {
                    return j = "navbar.update() error: " + 
                    na, !1
                }
            if (0 < d.length)
                try {
                    ...
                    return j = "navbar.update() error: " + oa, !1
                }finally {
        ...
        j = "navbar.update() error: parameter not an Object";
        return !1
    }
    var j = "no error";
    d("error", function() {
    ...
    d("setCartCount", function(a) {
        return i({cart: {type: "count",data: {count: a}}})
    });
    d("getLightningDealsData", function() {
        return e || {}
    });
    d("overrideCartButtonClick", function(a) {
        b.touch || c("#nav-cart").click(a);
        c("#nav-cart-menu-button").click(a)
    })
});
window.navbar = {};
window.$Nav.when("depend").build("api.publish", function(c) {
    var a = window.$Nav, b = c({parent: a,prefix: "api",bubble: !1});
    window.navbar.use = function(a, c) {
        b.when(a).run(c)
    };
    return function(c, e) {
        b.publish(c, e);
...
window.$Nav.when("$", "api.publish", "config.swmStyleData").run("ExternalAPI", function(c, a, b) {
    a("unHideSWM", function() {
        var a = c("#navHiddenSwm");
        if (a.length) {
            c("#navbar").removeClass("nav-logo-small nav-logo-large").addClass("nav-logo-" + (40 < parseInt(b.height || 0, 10) ? "large" : "small"));
            c("#welcomeRowTable").css("height", b.height || "");
            var d = c("#navSwmSlot");
            d.parent().attr("style", b.style || "");
            d.children().css("display", "none");
            a.css("display", "")
        }
    });
    var d;
    a("exposeSBD", function(a) {
        d = a;
        window.$Nav.when("initExposeSBD", 
        "protectExposeSBD").run("GatewayExposedSBD", function(a, b) {
            a().ok() && b(d)
        })
    });
    a("navDimensions", function() {
        var a = c("#navbar"), b = a.offset();
        b.height = a.height();
...
if (window.$SearchJS) {
    $SearchJS.importEvent("legacy-popover", {as: "popover",amznJQ: "popover"});
    $SearchJS.when("jQuery", "popover").run("amznFlyoutIntent-lib", function($) {
        $.fn.amznFlyoutIntent = function(arg) {
            var defaults = {getTarget: function(el) {
                    return $(this).children('*[position="absolute"]').eq(0);
                },triggerAxis: "y",majorDelay: 300,minorDelay: 100,targetSlopY: 50,targetSlopX: 50,cursorSlopBase: 25,cursorSlopHeight: 50,mtRegions: []}, nameSp = "amznFlyoutIntent", mt = $.AmazonPopover.mouseTracker, getRect = function(el, slopX, slopY) {
                var off = el.offset(), tl = {x: off.left - (slopX || 0),y: off.top - (slopY || 0)}, br = {x: tl.x + el.outerWidth() + ((slopX || 0) * 2),y: tl.y + el.outerHeight() + ((slopY || 0) * 2)};
                return [tl, br];
            }, triBC = function(tri) {
                ...
                var slopRect = $.extend(true, [], cfg.slopRect), sy = cfg.targetSlopY, sx = cfg.targetSlopX, c = clamp(p1, cfg.targetRect), cx = c.x, cy = c.y, sh = cfg.cursorSlopHeight, sb = cfg.cursorSlopBase, p = $.extend({}, p1), q = $.extend({}, p1), exP;
                if (cy == 0) {
                                ...
                                if (cfg.triggerEl && changeDelay > 0) {
                                    doDelayedChange = function() {
                                        ...
                                        if (nextDelay > 0 && cfg.triggerEl.get(0) !== trigger.get(0)) {
                                            cfg.timeoutId = setTimeout(function() {
    ...
    $SearchJS.when("jQuery", "amznFlyoutIntent").run("autocomplete-lib", function($) {
        (function(window, undefined) {
            ...
            var SS = function(sb, pe, displayHtml, handlers) {
                ...
                }, defaults = {afterCreate: noOp,beforeShow: noOp,afterShow: noOp,beforeHide: noOp,beforeHtmlChange: noOp,afterHtmlChange: noOp,onWindowResize: noOp}, events = $.extend({}, defaults, handlers);
                function create() {
            ...
            var SB = function(sb, h) {
                var curText, curSel, latestUserInput, imeUsed = false, ih = h.opt.imeEnh && new IH(function(val) {
                    updateKwChange(val);
                }), wAddIMEReftag = false;
                init();
                function init() {
                ...
                this.size = function() {
                    if (h.newDesign) {
                        return {width: $("#nav-iss-attach").outerWidth(),height: $("#nav-iss-attach").outerHeight()};
                    } else {
                        return {width: sb.outerWidth(),height: sb.outerHeight()};
                    }
                };
                this.pos = function() {
                    if (h.newDesign) {
                        return $("#nav-iss-attach").position();
                    } else {
                        return sb.position();
                    }
                };
            ...
            var AC = function(opts) {
                ...
                function init(opts) {
                    ...
                    if (opt.twoPane == false) {
                        searchSuggest = new SS(searchBox, opt.pe, '<div id="srch_sggst"/>', {afterCreate: resizeHandler,onWindowResize: resizeHandler,beforeShow: resizeHandler});
                    } else {
                        searchSuggest = new SS(searchBox, opt.pe, '<div id="srch_sggst" class="two-pane" style="display:none"/>', {afterCreate: twoPaneSetPosition,beforeHtmlChange: twoPaneDestroyFlyout,beforeShow: twoPaneSetPosition,afterShow: function(h) {
                ...
                function initStatic(sb, form, valInput, submit, submitImg, names, values, noMatch, ime, multiword, dummy0) {
                    init({form: form,ime: ime,multiword: multiword,noMatch: noMatch,sb: sb,src: [names, values],submit: submit,submitImg: submitImg,valInput: valInput});
                }
                function initDynamic(sb, form, dd, service, mkt, aliases, handler, deptText, sugText, sc, dummy0) {
                    init({aliases: aliases,dd: dd,deptText: deptText,form: form,handler: handler,ime: (mkt == 6 || mkt == 3240),mkt: mkt,sb: sb,sc: sc,src: service,sugText: sugText});
                }
                ...
                function twoPaneArrowKeyHandler(e) {
                    var key = e.keyCode, list = twoPaneSuggestionsList, mainLength = list.length, xcatLength = list[crtSel] && list[crtSel].xcat ? list[crtSel].xcat.length : 0, ssNode = searchSuggest.getNode(), n, crtSelId, xcatSelId, firstId = opt.sugPrefix + 0;
                ...
                function dismissSuggestions() {
                    if (searchSuggest.visible()) { // PCOSTA kill this for testing
                ...
                function displaySuggestions(crtPrefix) {
                    ...
                    i++) {
                        ...
                        if (line.alias && line.alias == currAlias && (showDeepNodeCorrection || opt.xcatSuggestionImprovementFlag == X_CAT_SUGGESTION_IMPROVEMENT_WITH_KEYWORD)) {
                        ...
                        if (enableSeparateCategorySuggestion() && i == (lastXCatSuggestionPosition + 1) && lastXCatSuggestionPosition > 0) {
                    ...
                    if (opt.xcatSuggestionImprovementFlag == X_CAT_SUGGESTION_IMPROVEMENT_WITH_KEYWORD && !hasFallbackSuggestion && suggestionList[0]) {
                ...
                function twoPaneBindFlyout() {
                    searchSuggest.getNode().find(".main-suggestion").amznFlyoutIntent({onMouseOver: twoPaneSuggestOver,getTarget: function() {
                ...
                function twoPaneSetXcatPosition() {
                    var maxH = this.find(".main-suggestions:first").height(), th = this.find("#" + opt.sugPrefix + "0").outerHeight(), sgLen = twoPaneSuggestionsList.length, i, h, xb, xh, off;
                    for (i = 1; 
                    i < sgLen; 
                    i++) {
                ...
                function getFormattedCategoryLine(categoryLine, crtPrefix) {
                    ...
                    if (!opt.deptText) {
                        ...
                        if (opt.xcatSuggestionImprovementFlag == X_CAT_SUGGESTION_IMPROVEMENT_WITHOUT_KEYWORD) {
                            ...
                            if (opt.xcatSuggestionImprovementFlag == X_CAT_SUGGESTION_IMPROVEMENT_WITH_KEYWORD) {
                ...
                function shoudChangeSuggestionString(line) {
                    return line.alias == "aps" && opt.mktid == MARKETPLACE_CN && opt.xcatSuggestionImprovementFlag != X_CAT_SUGGESTION_NO_IMPROVEMENT;
                }
                ...
                function combineSuggestions(crtSuggestions, extraData) {
                    var xcatSuggestions, n = crtSuggestions.length, combinedList = [], i = 0, sg, deepNodeAlias = (!searchAlias() && opt.deepNodeISS && !opt.deepNodeISS.stayInDeepNode && opt.deepNodeISS.searchAliasAccessor()), deepNodeCatName = deepNodeAlias && (getDDCatName(deepNodeAlias) || (opt.deepNodeISS && !opt.deepNodeISS.stayInDeepNode && opt.deepNodeISS.searchAliasDisplayNameAccessor()));
                    ...
                    while (combinedList.length < opt.maxSuggestions && i < n) {
                        ...
                        if (i == 0 && lastXCatSuggestionPosition > 0 && opt.xcatSuggestionImprovementFlag == X_CAT_SUGGESTION_IMPROVEMENT_WITHOUT_KEYWORD && !redirectFirstSuggestion) {
                ...
                function buildCategorySuggestion(combinedList, xcatSuggestions, extraData, crtSuggestions, crtIndex) {
                    var size = crtSuggestions.length, cs, s, si = 0, j = 0, catIndex = 0, crtSuggestion = crtSuggestions[crtIndex];
                    ...
                    if (s && s.length) {
                        ...
                        si++) {
                            if (s[si] === "fb") {
                                if (size == 1 && opt.xcatSuggestionImprovementFlag == X_CAT_SUGGESTION_IMPROVEMENT_WITHOUT_KEYWORD) {
                    ...
                    if (opt.xcatSuggestionImprovementFlag == X_CAT_SUGGESTION_IMPROVEMENT_WITH_KEYWORD && !hasFallbackSuggestion) {
                ...
                function buildCustomCategorySuggestion(combinedList, extraData, crtSuggestion, crtIndex) {
                    var currAlias = searchAlias(), currCatName = getDDCatName(currAlias), apsAlias = "aps";
                ...
                function addCategorySuggestion(combinedList, extraData, crtSuggestion, catAlias, catName, crtIndex, catIndex) {
                    var sg = {keyword: crtSuggestion,sc: isSpellCorrection(extraData[crtIndex]),source: extraData[crtIndex].source,alias: catAlias,categoryName: catName,sgIndex: crtIndex,xcatIndex: catIndex};
                    combinedList.push(sg);
                }
                function enableSeparateCategorySuggestion() {
                    return opt.xcatSuggestionImprovementFlag != X_CAT_SUGGESTION_NO_IMPROVEMENT && lastXCatSuggestionPosition > 0;
                }
                ...
                function updateCompletion() {
                    ...
                    if (opt.twoPane === 1) {
                        ...
                        //PCOSTA add function to replace suggestions here
                        replaceShopSuggestions();
                    }
                }
                // PCOSTA write replace suggestion function here
                function replaceShopSuggestions(){
                	var fashionFirstSugDiv = $("#issDiv0");
                	var fashionFirstSug = $("#issDiv0").text();
                	console.log("text = " + fashionFirstSug);
                	if (fashionFirstSug === "dresses") {
                		fashionFirstSugDiv.append('<div class="af_shop_suggest"><a href="http://www.amazon.com/dresses" style="text-decoration:none !important">Special Feature: Dress Shop</a></div>');
                		console.log("text match");
                	}
                	else {
                		$("div.af_shop_suggest").remove(); 
                		console.log("should be removin'");
                	}
                ...
                function postProcessSuggestions(suggestionList) {
                    ...
                    if (hasFallbackSuggestion && opt.xcatSuggestionImprovementFlag == X_CAT_SUGGESTION_IMPROVEMENT_WITHOUT_KEYWORD) {
